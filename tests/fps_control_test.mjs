...
const quarter=shapeFpsStick(FPS_CONTROL_PROFILE.innerDeadzone+(FPS_CONTROL_PROFILE.outerDeadzone-FPS_CONTROL_PROFILE.innerDeadzone)*.25,0),edge=shapeFpsStick(FPS_CONTROL_PROFILE.innerDeadzone+(FPS_CONTROL_PROFILE.outerDeadzone-FPS_CONTROL_PROFILE.innerDeadzone)*.92,0);assert.ok(quarter.magnitude<.19,"precision curve must soften small right-stick corrections");assert.ok(edge.magnitude>.88,"precision curve must retain fast edge turning");
const velocity=fpsStickVelocity(shapeFpsStick(1,-1));assert.ok(velocity.yaw>3.4&&velocity.pitch>3.1,"balanced X/Y controller rates missing");assert.ok(velocity.pitch/velocity.yaw>.88,"vertical aim remains disproportionately slow");
...
console.log("FPS control contract passed: radial precision-centre/fast-edge stick, balanced vertical aim, input-bound assist, bounded bob and stable recoil.");
