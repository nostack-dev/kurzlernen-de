...
for(const marker of ["THREE.Raycaster","SHOT_INTERVAL_MS","BLOCKED_SELECTOR","DECAL_POOL_SIZE=32","fireDecalPoolSize","touch-action:none","setPointerCapture","flightFireDecal"])
...
try{
  ...
  if(!mobileUx.logbook||!mobileUx.fire||mobileUx.decalPool!==32)throw new Error(`solo logbook/fire/decal-pool UI missing: ${JSON.stringify(mobileUx)}`);
  ...
  // Hold and drag on the free flight surface. Moving the live touch must move the
  // aim point and the automatic fire cadence must continue until pointerup.
  const dragFire=await page.evaluate(async()=>{const v=document.querySelector("#viewport"),r=v.getBoundingClientRect(),sx=r.left+r.width*.50,sy=r.top+r.height*.48,ex=sx+34,ey=sy+28,send=(type,x,y)=>v.dispatchEvent(new PointerEvent(type,{bubbles:true,cancelable:true,pointerId:81,pointerType:"touch",clientX:x,clientY:y,button:0})),before=Number(v.dataset.fireShots||0);send("pointerdown",sx,sy);await new Promise(resolve=>setTimeout(resolve,125));send("pointermove",ex,ey);await new Promise(resolve=>setTimeout(resolve,360));const during=Number(v.dataset.fireShots||0),left=parseFloat(document.querySelector("#flightFireReticle")?.style.left||"0"),top=parseFloat(document.querySelector("#flightFireReticle")?.style.top||"0");send("pointerup",ex,ey);return{before,during,left,top,centerX:r.width/2,centerY:r.height/2,pool:Number(v.dataset.fireDecalPoolSize||0)};});
  if(dragFire.during-dragFire.before<4||dragFire.pool!==32||Math.hypot(dragFire.left-dragFire.centerX,dragFire.top-dragFire.centerY)<20)throw new Error(`touch drag did not keep firing/follow aim: ${JSON.stringify(dragFire)}`);
  ...
  // Training THREE hits use a fixed 32-mesh decal pool. Stress beyond the pool
  // size and prove meshes are recycled instead of growing with shot count.
  ...
  console.log(`REAL WORLD E2E passed: exact 5/50m FPV eye pose, bounded FPV MapLibre cadence, shared north-up minimap, persisted 5-54 km/h GAME speed, drag-to-fire, fixed THREE decal pool and local flight logbook.`);
