...
try{
  ...
  const expanded=await page.evaluate(()=>{const h=document.querySelector("#worldLookHud"),r=h.getBoundingClientRect(),v=document.querySelector("#viewport");return{w:r.width,h:r.height,mode:v.dataset.worldMinimapMode,bearing:Number(v.dataset.worldMinimapBearing||99),projection:v.dataset.worldMinimapProjection};});if(expanded.w<250||expanded.h<200||expanded.mode!=="north"||Math.abs(expanded.bearing)>.01||expanded.projection!=="topdown")throw new Error(`expanded minimap contract failed: ${JSON.stringify(expanded)}`);
