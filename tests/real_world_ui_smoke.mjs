...
try{
  ...
  if(!(live.mapUpdates>0&&live.mapUpdates<live.frames)||![15,20,30].includes(live.mapFpsCap)||live.mapPixelRatio!==1||live.flightPixelRatio>1.25||!live.geoBackground.includes("gradient")||live.grid!=="1"||live.keepLook!=="0"||live.lookHud==="none"||live.legend==="none"||!["nominal","constrained","critical"].includes(live.perfMode))throw new Error(`WORLD render budget/contrast failed: ${JSON.stringify(live)}`);
