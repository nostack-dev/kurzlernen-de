...
for(const marker of ["THREE.Raycaster","SHOT_INTERVAL_MS","BLOCKED_SELECTOR","DECAL_POOL_SIZE=32","fireDecalPoolSize","touch-action:none","setPointerCapture","flightFireDecal","clientX:event.clientX","fireAimX","isArmed","fireLockReason"])
...
const waitWorld=async()=>{
  try{await page.waitForFunction(()=>{const v=document.querySelector("#viewport");return v?.dataset.worldMode==="real"&&v?.dataset.worldProvider==="openfreemap-esri-imagery"&&v?.dataset.worldImageryLayer==="ready"&&v?.dataset.worldImageryEnabled==="0"&&Number(v?.dataset.worldThreeFrames||0)>4;},{timeout:20000});}
...
try{
  ...
  if(!config.section||config.imagery!==false||config.grid!==true||config.keep!==false||config.axis!==true||config.axisDisabled!==await page.evaluate(()=>Boolean(document.fullscreenElement))||config.obsolete||!config.fovLabel||config.speed!==36||!config.speedText.includes("36 km/h")||!config.note.includes("No account, API key, billing setup, backend or proxy")||!config.note.includes("OFF by default"))throw new Error(`WORLD/settings contract failed: ${JSON.stringify(config)}`);
  ...
  await page.click("#soloTopbar .phone-settings-button");await page.waitForFunction(()=>document.querySelector(".phone-settings-dialog")?.open,{timeout:5000});
  await page.click('.phone-settings-dialog [data-world-imagery]');await page.click('.phone-settings-dialog [data-close]');await page.waitForFunction(()=>localStorage.getItem("arondight45WorldImageryV1")==="1"&&Number(document.querySelector("#viewport")?.dataset.worldMinimapImageryTiles||0)>0,{timeout:10000});
  ...
  await page.waitForFunction(()=>{const button=document.querySelector("#soloArm");return button&&!button.disabled&&button.textContent.trim()==="ARM";},{timeout:20000});await page.click("#soloArm");await page.waitForFunction(()=>document.querySelector("#fcState")?.textContent==="ARMED"&&document.querySelector("#viewport")?.dataset.fireArmed==="1",{timeout:65000});
