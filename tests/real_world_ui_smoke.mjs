...
try{
  ...
  // aim point and the automatic fire cadence must continue until pointerup. In
  // WORLD the same fixed THREE decal pool must receive actual world-anchored hits.
  const dragFire=await page.evaluate(async()=>{const v=document.querySelector("#viewport"),r=v.getBoundingClientRect(),sx=r.left+r.width*.50,sy=r.top+r.height*.48,ex=sx+34,ey=sy+28,send=(type,x,y)=>v.dispatchEvent(new PointerEvent(type,{bubbles:true,cancelable:true,pointerId:81,pointerType:"touch",clientX:x,clientY:y,button:0})),before=Number(v.dataset.fireShots||0),writes0=Number(v.dataset.fireDecalWrites||0);send("pointerdown",sx,sy);await new Promise(resolve=>setTimeout(resolve,125));send("pointermove",ex,ey);await new Promise(resolve=>setTimeout(resolve,360));const during=Number(v.dataset.fireShots||0),writes=Number(v.dataset.fireDecalWrites||0),left=parseFloat(document.querySelector("#flightFireReticle")?.style.left||"0"),top=parseFloat(document.querySelector("#flightFireReticle")?.style.top||"0");send("pointerup",ex,ey);let worldDecals=0;globalThis.__arondightRealWorld?.threeScene?.traverse?.(node=>{if(node.userData?.flightFireDecal&&node.userData?.flightFireWorld)worldDecals++;});return{before,during,writes0,writes,worldDecals,left,top,centerX:r.width/2,centerY:r.height/2,pool:Number(v.dataset.fireDecalPoolSize||0)};});
  if(dragFire.during-dragFire.before<4||dragFire.pool!==32||dragFire.writes<=dragFire.writes0||dragFire.worldDecals<1||Math.hypot(dragFire.left-dragFire.centerX,dragFire.top-dragFire.centerY)<20)throw new Error(`WORLD touch drag did not keep firing/follow aim/write pooled decals: ${JSON.stringify(dragFire)}`);
  ...
  console.log(`REAL WORLD E2E passed: exact FPV camera registration, shared north-up minimap, persisted 5-54 km/h GAME speed, WORLD drag-to-fire, and one recycled 32-mesh THREE decal pool across WORLD/TRAINING.`);
