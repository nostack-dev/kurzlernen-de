...
try{
  ...
  if(!config.section||config.imagery!==false||config.grid!==true||config.keep!==false||config.axis!==true||config.axisDisabled!==Boolean(await page.evaluate(()=>document.fullscreenElement))||config.obsolete||!config.fovLabel||config.speed!==36||!config.speedText.includes("36 km/h")||!config.note.includes("No account, API key, billing setup, backend or proxy")||!config.note.includes("OFF by default"))throw new Error(`WORLD/settings contract failed: ${JSON.stringify(config)}`);
