...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const afterRespawn={kills:bridge.vsKills,peerHp:bridge.vsPeerHealth,peerDead:bridge.vsPeerDead,hud:document.querySelector("#vsCombatHud")?.textContent||""};
    for(let i=0;i<4;i++){dispatchEvent(new CustomEvent("arondight45:vs-game",{detail:{peerId:"peer-b",packet:{type:"hit-request",id:`local-death-${i}`,shooter:"peer-b",target:"peer-a",damage:25}}}));await sleep(30);}
    ...
    const localDeath={deaths:bridge.vsDeaths,hp:bridge.vsLocalHealth,dead:bridge.vsLocalDead,hud:document.querySelector("#vsCombatHud")?.textContent||"",state:sentGame.findLast?.(item=>item.packet.type==="state"&&item.packet.playerId==="peer-a"&&item.packet.hp===0)?.packet||[...sentGame].reverse().find(item=>item.packet.type==="state"&&item.packet.playerId==="peer-a"&&item.packet.hp===0)?.packet};
    return{beforeRespawn,afterRespawn,localDeath,killState:sentGame.find(item=>item.packet.type==="state"&&item.packet.playerId==="peer-c"&&item.packet.hp===0)?.packet,respawn:sentGame.find(item=>item.packet.type==="respawn"&&item.packet.playerId==="peer-c")?.packet,explosion:sentFx.some(item=>item.packet.type==="explosion"&&item.packet.playerId==="peer-c")};
  });
  ...
  console.log("VS multiplayer score browser smoke passed: host-authoritative K/D increments once, explosion broadcasts, respawn restores HP, and K score persists.");
