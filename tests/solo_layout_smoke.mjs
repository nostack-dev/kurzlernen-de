  ...
  for(const viewport of [{width:844,height:390,name:"landscape"},{width:844,height:300,name:"safari-bars"},{width:390,height:844,name:"iphone-portrait"}]){
    ...
    const g=await page.evaluate(()=>{
      ...
      return{
        ...
        topbar:rect("#soloTopbar"),topbarActions:rect("#soloTopbarActions"),topbarStatus:rect("#soloTopbarStatus"),toolbarLayout:document.querySelector("#soloTopbar")?.dataset.toolbarLayout||"",toolbarButtons:[...document.querySelectorAll("#soloTopbarActions>button")].filter(e=>getComputedStyle(e).display!=="none").map(e=>({id:e.id||e.className,rect:logicalElementRect(e)})),vs:rect("#lanVsButton"),vsParent:document.querySelector("#lanVsButton")?.parentElement?.id||"",vsCombatParent:document.querySelector("#vsCombatHud")?.parentElement?.id||"",vsCombatHidden:Boolean(document.querySelector("#vsCombatHud")?.hidden),left:rect("#soloLeft"),right:rect("#soloRight"),clearance:rect("#soloClearance"),arm:rect("#soloArm"),kill:rect("#soloKill"),
    ...
    for(const key of ["topbar","topbarActions","topbarStatus","vs","left","right","clearance","arm","kill"])if(!g[key])throw new Error(`${viewport.name}: missing ${key}`);
