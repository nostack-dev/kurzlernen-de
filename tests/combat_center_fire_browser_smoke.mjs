...
try{
  ...
  await page.waitForFunction(required=>{const v=document.querySelector("#viewport"),markers=String(v?.dataset.combatHitStackWorldMarkers||"");return v?.dataset.worldProceduralPopulation==="1"&&v?.dataset.combatHitStackRegistry==="marker-union-v1"&&required.every(marker=>markers.includes(marker))&&Number(v?.dataset.combatHitStackStableFrames||0)>=12;},{timeout:8000},required);
  ...
  console.log("Hitscan browser smoke passed: full WORLD/procedural target stack converges, stays stable and sustains center-target fire without handler growth.");
