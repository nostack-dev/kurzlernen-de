...
try{
  ...
  if(stack.registry!=="marker-union-v1"||!stack.worldStable||stack.assignmentsAfter!==stack.assignmentsBefore||required.some(marker=>!stack.markers.includes(marker)))throw new Error(`target hit stack still mutates over time: ${JSON.stringify(stack)}`);
  ...
  console.log("Hitscan browser smoke passed: zero projectile pool, immediate raycast, stable marker-union target stack and sustained target fire without handler growth.");
