...
try{
  ...
  if(!during.active||during.throttle<.45||during.steer<.2||during.speedKmh<=0||!during.gasHeld)throw new Error(`real multitouch driving did not reach vehicle controller: ${JSON.stringify({positioned,boxes,during})}`);
  ...
  console.log(`Mobile car touch smoke passed: ${during.speedKmh.toFixed(1)} km/h, steer=${during.steer.toFixed(2)}, parkedSource=${positioned.parked}.`);
