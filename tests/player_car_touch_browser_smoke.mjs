...
try{
  ...
  if(!during.active||during.throttle<.45||during.steer<.2||during.speedKmh<=0||!during.gasHeld||!Number.isFinite(Number(beforePose?.yaw))||cumulativeYaw>-.015||cumulativeYaw< -2.2||clockwiseSamples<2||maxAbsAngularZ>2.2||during.headingSource!=="box3d-body-yaw-v1"||during.steeringPhysics!=="box3d-bicycle-yaw-rate-v1")throw new Error(`right touch steer did not produce stable clockwise Box3D steering: ${JSON.stringify({setup,boxes,beforePose,samples,cumulativeYaw,clockwiseSamples,maxAbsAngularZ})}`);
