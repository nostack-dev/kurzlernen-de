  ...
  await page.evaluate(()=>{
    ...
    renderer.render=(scene,camera)=>{
      ...
      const sample=(node,bucket)=>{
        ...
        for(const p of points){node.localToWorld(p);const cam=p.clone();camera.worldToLocal(cam);const projected=p.clone().project(camera);if(cam.z<-Math.max(.0001,Number(camera.near)||.01))front=true;if(cam.z<0&&projected.x>=-1&&projected.x<=1&&projected.y>=-1&&projected.y<=1&&projected.z>=-1&&projected.z<=1)onScreen=true;if(!best||Math.abs(projected.x)+Math.abs(projected.y)<Math.abs(best.ndc[0])+Math.abs(best.ndc[1]))best={name:node.name,cam:[cam.x,cam.y,cam.z],ndc:[projected.x,projected.y,projected.z]};}
