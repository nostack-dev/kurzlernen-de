...
const strategyOne=transportHarness("one"),strategyTwo=transportHarness("two"),shared=tapA[0],strategies=[{name:"One",load:strategyOne.loadTransport},{name:"Two",load:strategyTwo.loadTransport}];let faPeer=0,fbPeer=0,faPose=null,fbPose=null,faLeft=0,faTransport="",fbTransport="";
const fa=new LanVsFinder({transportStrategies:strategies,gestureDeferMs:0,joinDiagnosticMs:0,onPeer:(_peer,_room,transport)=>{faPeer++;faTransport=transport;},onPose:p=>faPose=p,onLeave:()=>faLeft++}),fb=new LanVsFinder({transportStrategies:strategies,gestureDeferMs:0,joinDiagnosticMs:0,onPeer:(_peer,_room,transport)=>{fbPeer++;fbTransport=transport;},onPose:p=>fbPose=p});
...
console.log("LAN VS deterministic smoke passed: Trystero 0.25 callback API, parallel signaling, 0-GPS rendezvous, network diagnostics, 30 Hz timestamped position/velocity/frame poses, origin, combat and shared beacon packets.");
