...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const personId=personMesh.userData.worldPopulationId;if(!bridge.registerWorldPopulationHit({object:personMesh}))throw Error("pedestrian shot was not accepted");
    await waitFor(()=>Number(view.dataset.worldRagdolls||0)>=1&&Number(view.dataset.worldRagdollParts||0)>=14&&bridge.threeScene.children.some(node=>node.userData?.worldRagdollRoot&&node.visible));
    ...
    const before=parts.map(item=>[item.node.position.x,item.node.position.y,item.node.position.z]);await sleep(480);const after=parts.map(item=>[item.node.position.x,item.node.position.y,item.node.position.z]);let maxMove=0;for(let i=0;i<Math.min(before.length,after.length);i++)maxMove=Math.max(maxMove,Math.hypot(after[i][0]-before[i][0],after[i][1]-before[i][1],after[i][2]-before[i][2]));
    const visibleOriginal=Boolean(personGroup.visible),pool=Number(view.dataset.worldRagdollPool||0),maxPool=Number(view.dataset.worldRagdollMax||0),spawns=Number(view.dataset.worldRagdollSpawns||0),active=Number(view.dataset.worldRagdolls||0),partCount=Number(view.dataset.worldRagdollParts||0),lastId=view.dataset.worldRagdollLastId||"";bridge.active=false;return{personId,visibleOriginal,pool,maxPool,spawns,active,partCount,lastId,maxMove};
  });
  ...
  console.log("WORLD pedestrian ragdoll browser smoke passed: iPad pool=6, 14 articulated parts, shot impulse/gravity motion, and immediate corpse handoff.");
