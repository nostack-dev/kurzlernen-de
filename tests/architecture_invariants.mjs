...
requireText("sim/simulator.mjs","SIM_MAX_BACKLOG_MS = 2000");
requireText("sim/simulator.mjs","SIM_MAX_STEPS_PER_SLICE = Math.ceil(SIM_MAX_CATCHUP_MS / SIM_FIXED_STEP_MS)");
requireText("sim/simulator.mjs","pendingMs=accumulatorMs+elapsedMs");
requireText("sim/simulator.mjs","simulationTimingDiscontinuityMs+=pendingMs-SIM_MAX_BACKLOG_MS");
requireText("sim/simulator.mjs","accumulatorMs=Math.min(pendingMs,SIM_MAX_BACKLOG_MS)");
...
requireText("sim/simulator.mjs","workElapsedMs=Math.max(0,afterWork-schedulerWallMs)");
...
requireText("sim/simulator.mjs","waitForSimulationDeadline=accumulatorMs=>new Promise");
requireText("sim/simulator.mjs","if(accumulatorMs>=SIM_FIXED_STEP_MS)await yieldToBrowser();else await waitForSimulationDeadline(accumulatorMs)");
...
requireText("sim/simulator.mjs","PRESENTATION_MAX_DRAW_GAP_MS = 48");
...
requireText("sim/simulator.mjs","PRESENTATION_SOFTWARE_PIXEL_RATIO = .30");
...
requireText("sim/simulator.mjs","presentationQualityCeiling=presentationSoftwareRaster?Math.min(presentationNativePixelRatio,PRESENTATION_SOFTWARE_PIXEL_RATIO):presentationNativePixelRatio");
requireText("sim/simulator.mjs","presentationPixelRatio<presentationQualityCeiling");
requireText("sim/simulator.mjs","renderer.shadowMap.enabled=!presentationSoftwareRaster");
requireText("sim/simulator.mjs","softwareRasterDrawInterval=0");
requireText("sim/simulator.mjs","effectiveDrawInterval=Math.max(minDrawInterval,softwareRasterDrawInterval)");
requireText("sim/render_stability.mjs","swiftshader|llvmpipe|software raster|software renderer");
...
requireText("sim/simulator.mjs","presentationTiming:{get:()=>presentationTimingSnapshot()");
requireText("sim/simulator.mjs","simulationTimingDiscontinuityMs:{get:()=>simulationTimingDiscontinuityMs");
requireText("sim/simulator.mjs","b3.b3World_Step(this.world,dt,4)");
requireText("sim/simulator.mjs","this.cdA=[.035*p.dragScale,.035*p.dragScale,.07*p.dragScale]");
requireText("sim/simulator.mjs","cdA=this.cdA");
...
requireText("sim/simulator.mjs",'FITTED_PHYSICS_STORAGE="arondight45FittedPhysicsV3"');
requireText("sim/simulator.mjs",'LEGACY_FITTED_PHYSICS_STORAGE="arondight45FittedPhysicsV2"');
requireText("sim/simulator.mjs","partitionCalibrationLog(realLog)");
requireText("sim/simulator.mjs","evaluatePhysicsValidation({allSamples:realLog,trainingSamples:partition.training,holdoutSamples:partition.holdout");
requireText("sim/simulator.mjs",'if(raw===""||raw===undefined){row[header]=null',"blank CSV measurements must never become fake zero-valued evidence");
requireText("sim/simulator.mjs","Fit applied but UNVALIDATED");
requireText("drone_simulator.html","UNVALIDATED · nominal coefficients");
for(const marker of ["minimumHoldoutDurationS","collectiveMotorSpanUs","positionRmseM","currentRmseA","holdoutNrmseGrowth"])requireText("sim/physics_validation.mjs",marker);
for(const marker of ["does **not** yet have a 1:1 ESP32-S31","Functional MCU proof; board run still required","Terrain/building physics","ICM-42688-P temperature","70% of each ordered log"])requireText("S31_DIGITAL_TWIN_AUDIT.md",marker);
...
for(const marker of ["ANDROID_RE=/android/i","HONOR_RE=","pixelRatioCeiling=software?Math.min(nativePixelRatio,.30):android?Math.min(nativePixelRatio,1)","stableBackbuffer:android||honor","canvasDesynchronized:false","quantizedViewportSize"])requireText("sim/render_stability.mjs",marker);
forbidText("sim/real_world_bootstrap.mjs","desynchronized:true","WORLD must not bypass the compositor under a transparent WebGL/HUD stack");
requireText("sim/real_world_bootstrap.mjs",'desynchronized:false,preserveDrawingBuffer:false');
...
requireText("sim/simulator.mjs","gamepadconnected");
requireText("sim/simulator.mjs","XBOX ON · PRESS ANY BUTTON");
requireText("sim/simulator.mjs","XBOX DETECTED · ENABLE IN SETTINGS");
...
requireText("sim/real_world_bootstrap.mjs","this.vsSession.setOrigin(origin)","GPS-capable peers must publish the canonical world origin");
...
for(const marker of ['import {VsPoseTimeline,normalizeVsOrigin,chooseCanonicalVsOrigin,poseMatchesVsFrame,vsFrameId,vsOriginKey} from "./vs_pose_sync.mjs";','chooseCanonicalVsOrigin(this.vsLocalOriginCandidate,this.vsSharedOrigin,remote)','applyVsWorldOrigin(origin,fromMate)','restoreVsLocalWorld()','this.clearVsPeerPresentation();','poseMatchesVsFrame(pose,this.vsSharedOrigin)','this.vsPeerTimeline.push(packet,received)','this.vsPeerTimeline.sample(performance.now())','f:vsOriginKey(this.vsSharedOrigin)||"local-metric"'])requireText("sim/real_world_bootstrap.mjs",marker);
for(const marker of ["VS_POSE_INTERPOLATION_DELAY_MS=90","VS_POSE_MAX_EXTRAPOLATION_MS=100","VS_POSE_STALE_HOLD_MS=3000","class VsPoseTimeline","mode:\"interpolate-predict\"","velocity[index]*predictionMs/1000","mode:extrapolationMs>0?\"extrapolate\":\"hold\""])requireText("sim/vs_pose_sync.mjs",marker);
for(const marker of ['vsFrameId(origin)','pose.f===vsFrameId(origin)'])requireText("sim/vs_pose_sync.mjs",marker);
forbidText("sim/vs_pose_sync.mjs","localeCompare","canonical VS frame election must be byte-deterministic across different phone locales");
forbidText("sim/real_world_bootstrap.mjs","this.vsPeerRenderPosition.lerp","frame-rate-dependent exponential peer chasing must not return");
requireText("sim/lan_vs.mjs","const SEND_MS=33;","direct owner-authoritative pose stream must run at about 30 Hz");
requireText("sim/lan_vs.mjs","...(pose.v?{v:[...pose.v]}:{})","pose velocity must cross the network as a copied physical vector");
