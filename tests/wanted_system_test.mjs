...
assert.equal(wantedPoliceCount(0),0);
assert.equal(wantedPoliceCount(1),2);
assert.equal(wantedPoliceCount(2),3);
...
assert.ok(wantedPoliceArrivalDelayMs(1)>=2200&&wantedPoliceArrivalDelayMs(1)<=2500&&wantedPoliceArrivalDelayMs(5)>=1800&&wantedPoliceArrivalDelayMs(5)<wantedPoliceArrivalDelayMs(1));
assert.ok(wantedPoliceEngageDelayMs(1)>=1900&&wantedPoliceEngageDelayMs(5)>=1500&&wantedPoliceEngageDelayMs(5)<wantedPoliceEngageDelayMs(1));
...
console.log("Wanted-system logic passed: denser dispatch, hard fire range, 30 m EMP, staggered altitude, wave breaks, imperfect movement-aware accuracy and escape are deterministic.");
