...
try{
  ...
  const result=await page.evaluate(async()=>{
    ...
    const EARTH_RADIUS_M=6378137,latRad=b.originLat*Math.PI/180,cosLat=Math.max(.01,Math.cos(latRad));
    ...
    const run=async everyFrame=>{
      ...
      return{calls,updates:b.mapUpdates-updates0,p95EyeM:eyeLag.sort((a,b)=>a-b)[Math.floor(eyeLag.length*.95)],p95Pixel:pixelLag.sort((a,b)=>a-b)[Math.floor(pixelLag.length*.95)],p95BearingDeg:bearingLag.sort((a,b)=>a-b)[Math.floor(bearingLag.length*.95)]};
    };
    const locked=await run(true),halfRate=await run(false);
    ...
    return{locked,halfRate};
  });

  const {locked,halfRate}=result;
