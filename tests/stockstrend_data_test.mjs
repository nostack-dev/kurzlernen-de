...
assert(html.includes('id="sentiment3dToggle"'),'HDR 3D sentiment toggle missing');
assert(html.includes('id="hypeChart3d"'),'HDR 3D sentiment canvas missing');
assert(html.includes("SENTIMENT_3D_LS='alantu_sentiment_3d_v1'"),'HDR 3D toggle persistence missing');
assert(html.includes('function sentimentReliability(h)'),'3D Z reliability function missing');
assert(html.includes('.65*conf+.35*sampleScore'),'3D Z must combine confidence and sample density');
assert(html.includes('function sentiment3dRows(rows,range)'),'3D sentiment aggregation missing');
assert(html.includes("ctx.createLinearGradient(a.x,a.y,b.x,b.y)"),'HDR 3D line gradient missing');
assert(html.includes("ctx.fillText('Z = Belastbarkeit'"),'3D Z-axis meaning must be visible');
assert(html.includes("localStorage.setItem(SENTIMENT_3D_LS,sentiment3d?'1':'0')"),'3D view state must persist');
...
assert(html.includes("min:session.open,max:session.close"),'1T axis must span fixed 08:00-22:00 range');
