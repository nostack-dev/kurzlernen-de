...
assert(html.includes("function nyRegularSessionAt(value)"),'NYSE regular-session selector missing');
assert(html.includes("function latestNyRegularSession(arr,getAt)"),'latest NYSE regular-session selector missing');
assert(html.includes("ms<s.open||ms>s.close"),'1T must exclude pre/post market');
assert(html.includes("min:session.open,max:session.close"),'1T axis must span exact NYSE regular session');
assert(html.includes("session.key===liveSession.key"),'live quote must only append inside selected regular session');
