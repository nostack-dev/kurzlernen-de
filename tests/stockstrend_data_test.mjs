...
assert(html.includes("function berlinDayKey(value)"),'Germany-local 1T day selector missing');
assert(html.includes("if(range==='1d')return oneBerlinDay(arr,getAt);"),'1T must use one Berlin calendar day, not rolling 24h');
assert(html.includes("if(range==='1d'){\n    var span=Math.max(1,last-first)"),'1T axis must bind to selected Berlin day');
