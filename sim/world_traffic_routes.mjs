...
export function isDrivableTransportationFeature(feature){const p=feature?.properties||{},layer=feature?.layer||{},token=[p.class,p.subclass,p.type,p.service,p.brunnel,p.surface,layer.id,layer["source-layer"],feature?.sourceLayer].filter(Boolean).join(" ").toLowerCase();if(NON_DRIVABLE_RE.test(token))return false;const cls=roadFeatureClass(feature);return DRIVABLE_RE.test(cls)||(!cls&&String(layer["source-layer"]||feature?.sourceLayer||"").toLowerCase()==="transportation");}
