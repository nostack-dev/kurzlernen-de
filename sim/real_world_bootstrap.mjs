...
class RealWorldBridge{
  constructor(){
    ...
    this.vsSession=null;this.vsPeerMesh=null;this.vsConnected=false;this.vsStarting=false;this.installUi();this.installLookHud();this.installFreeLookSurface();this.installVsUi();queueMicrotask(()=>this.startVs());window.addEventListener(CAMERA_SETTINGS_EVENT,event=>{const value=Number(event.detail?.fpvFovDeg);if(Number.isFinite(value)){this.viewFovDeg=clamp(value,50,120);this.minimapLastDrawMs=-Infinity;}});
  }
  installUi(){
