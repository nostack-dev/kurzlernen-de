...
class RealWorldBridge{
  ...
  installUi(){
    ...
    const card=document.createElement("div");card.className="card real-world-card";card.innerHTML=`
      ...
      <div id="realWorldConfig" hidden>
        ...
        <div class="help">Esri World Imagery renders real aerial/satellite pixels; OpenFreeMap + OpenStreetMap add roads and 3D building footprints. No account, API key or billing setup is required. Imagery and OSM geometry are visual/geospatial context only; the flight controller, motors, sensors and rigid-body physics stay on the same local SI digital-twin path, and map geometry is never silently treated as collision truth.</div>
      </div>
  ...
  async createMap(longitude,latitude){
    if(this.map){this.geoContainer.hidden=false;this.map.resize();this.map.jumpTo({center:[longitude,latitude],zoom:19,pitch:55,bearing:0});this.setImageryEnabled(this.imageryEnabled);return this.map;}
