...
class RealWorldBridge{
  constructor(){
    this.active=false;this.loading=false;this.map=null;this.originLon=null;this.originLat=null;this.threeRenderer=null;this.threeScene=null;this.threeCamera=null;this.flightPixelRatio=null;this.flightShadowEnabled=null;this.mapPixelRatio=WORLD_MAP_PIXEL_RATIO;this.geoContainer=null;this.worldCard=null;this.savedBackground=null;this.savedFog=null;this.trainingObjects=new Set();this.frameVisibility=new Map();this.lastLocation=null;this.lastViewportSize="";this.lastMapSyncMs=-Infinity;this.lastMapView=null;this.realFrames=0;this.presentationFrameSerial=0;this.lastFpvSyncFrameSerial=-1;this.mapUpdates=0;this.gridEnabled=loadBool(WORLD_GRID_STORAGE,true);this.keepLookOrientation=loadBool(WORLD_KEEP_LOOK_STORAGE,false);this.minimapAxisLocked=loadBool(WORLD_MINIMAP_AXIS_LOCK_STORAGE,true);this.lookYawDeg=0;this.lookPitchDeg=0;this.lookDragging=false;this.gamepadLookActive=false;this.lookSnapping=false;this.lookPointer=null;this.lookFrameMs=performance.now();this.lookHud=null;this.lookReadout=null;this.minimapTitle=null;this.mapLegend=null;this.minimapCanvas=null;this.minimapCtx=null;this.minimapFeatures=[];this.minimapLayerIds=[];this.minimapLastQueryMs=-Infinity;this.minimapLastDrawMs=-Infinity;this.minimapQueries=0;this.minimapExpanded=false;this.minimapZoom=1;this.minimapPointers=new Map();this.minimapPinch=null;this.lastMinimapTapMs=0;this.viewFovDeg=loadCameraSettings().fpvFovDeg;this.lookSurfaceInstalled=false;this.worldShotPoint=new THREE.Vector3();this.worldShotNormal=new THREE.Vector3(0,0,1);this.worldShotHit={point:this.worldShotPoint,worldNormal:this.worldShotNormal};this.worldShotQueries=0;this.airframe=null;this.mapFrameMs=WORLD_MAP_FRAME_MS;this.perfMode="nominal";this.perfWindowStart=performance.now();this.perfFrames=0;this.perfGoodWindows=0;this.flightFps=60;
    this.vsSession=null;this.vsPeerMesh=null;this.vsConnected=false;this.vsStarting=false;this.vsSharedOrigin=null;this.vsSharedWorldAttempted=false;this.vsWorldFromMate=false;this.vsPeerTargetPosition=null;this.vsPeerTargetQuaternion=null;this.vsPeerRenderPosition=null;this.vsPeerRenderQuaternion=null;this.vsPeerLastPoseMs=-Infinity;this.vsPeerRenderMs=performance.now();this.vsCombatHud=null;this.vsLocalHealth=100;this.vsPeerHealth=100;this.vsKills=0;this.vsDeaths=0;this.vsCombatSeq=0;this.vsSeenHits=new Set();this.vsPendingHits=new Set();this.vsRespawnTimer=0;this.vsLocalDead=false;this.vsPeerDead=false;this.vsExplosion=null;this.vsExplosionStartedMs=-Infinity;this.installUi();this.installLookHud();this.installFreeLookSurface();this.installVsUi();window.addEventListener(CAMERA_SETTINGS_EVENT,event=>{const value=Number(event.detail?.fpvFovDeg);if(Number.isFinite(value)){this.viewFovDeg=clamp(value,50,120);this.minimapLastDrawMs=-Infinity;}});document.addEventListener("fullscreenchange",()=>{this.minimapLastDrawMs=-Infinity;this.renderLookHud();this.drawMinimap(performance.now());});
  }
  installUi(){
  ...
  async startVs(){
    ...
    try{const deviceCoords=!this.vsWorldFromMate&&Number.isFinite(this.originLon)&&Number.isFinite(this.originLat)?{longitude:this.originLon,latitude:this.originLat}:this.lastLocation?.coords&&Number.isFinite(this.lastLocation.coords.longitude)&&Number.isFinite(this.lastLocation.coords.latitude)?{longitude:this.lastLocation.coords.longitude,latitude:this.lastLocation.coords.latitude}:{};const roomIds=await discoveryRoomKeys(deviceCoords);const viewport=$("viewport");if(viewport)viewport.dataset.vsDiscoveryRooms=String(roomIds.length);if(!this.vsStarting)return;const session=new LanVsFinder({onTransport:()=>{if(button&&!this.vsConnected)button.textContent="FINDING…";},onPeer:()=>{this.vsConnected=true;if(button)button.textContent="MATE ✓";this.ensureVsPeerMesh();this.resetVsCombat(true);},onPose:pose=>this.applyVsPose(pose),onOrigin:origin=>this.acceptVsOrigin(origin,null),onCombat:packet=>this.applyVsCombat(packet),onLeave:()=>{const mateWorld=this.vsWorldFromMate;this.vsConnected=false;this.vsSharedOrigin=null;this.vsSharedWorldAttempted=false;this.vsWorldFromMate=false;this.clearVsPeerPresentation();this.resetVsCombat(false);if(mateWorld){if(this.active)this.deactivate();this.originLon=null;this.originLat=null;}if(button)button.textContent="WAITING…";},onError:()=>{if(button)button.textContent="RETRY MATE";}});this.vsSession=session;await session.start(roomIds);if(!this.vsConnected&&button)button.textContent="WAITING…";}catch(error){this.vsSession?.stop();this.vsSession=null;this.resetVsCombat(false);if(button)button.textContent="RETRY MATE";console.warn("VS discovery unavailable:",error);}finally{this.vsStarting=false;}
  }
  ...
  updateVsPeerRender(){
    this.updateVsExplosion();if(!this.vsPeerMesh||!this.vsPeerTargetPosition||!this.vsPeerTargetQuaternion)return;if(this.vsPeerDead){this.vsPeerMesh.visible=false;return;}const now=performance.now();
  ...
  stopVs(){this.vsStarting=false;const mateWorld=this.vsWorldFromMate;this.vsSession?.stop();this.vsSession=null;this.vsConnected=false;this.vsSharedOrigin=null;this.vsSharedWorldAttempted=false;this.vsWorldFromMate=false;this.clearVsPeerPresentation();this.resetVsCombat(false);if(mateWorld){if(this.active)this.deactivate();this.originLon=null;this.originLat=null;}const b=$("lanVsButton");if(b)b.textContent="FIND MATE";}
  ...
  installLookHud(){
    ...
    viewport.appendChild(hud);this.lookHud=hud;this.lookReadout=hud.querySelector("[data-world-look-readout]");this.minimapTitle=hud.querySelector("[data-world-minimap-title]");this.minimapCanvas=hud.querySelector(".world-mini-canvas");this.minimapCtx=this.minimapCanvas?.getContext("2d");const legend=document.createElement("div");legend.id="worldMapLegend";legend.innerHTML='<span class="legend-water"><i></i>WATER</span><span class="legend-green"><i></i>GREEN</span><span class="legend-road"><i></i>ROADS</span><span class="legend-building"><i></i>BUILDINGS</span>';viewport.appendChild(legend);this.mapLegend=legend;
  ...
  renderLookHud(){
    ...
    const viewport=$("viewport");if(viewport){viewport.dataset.worldLookYaw=this.lookYawDeg.toFixed(2);viewport.dataset.worldLookPitch=this.lookPitchDeg.toFixed(2);viewport.dataset.worldLookKeepEnabled=this.keepLookOrientation?"1":"0";viewport.dataset.worldGridEnabled=this.gridEnabled?"1":"0";viewport.dataset.worldMinimapMode=Math.abs(miniBearing)<.01?"north":"look";viewport.dataset.worldMinimapBearing=miniBearing.toFixed(2);viewport.dataset.worldMinimapProjection="topdown";viewport.dataset.worldMinimapPitch="0.00";viewport.dataset.worldMinimapRoll="0.00";viewport.dataset.worldMinimapHeightMode="flat-footprints";viewport.dataset.worldMinimapAxisLock=this.minimapAxisLocked?"1":"0";viewport.dataset.worldMinimapAxisLockApplied=axisLockApplies?"1":"0";}
  }
  ...
  drawMinimap(now){
    ...
    for(const feature of this.minimapFeatures){for(const path of feature.paths){const footprint=path.map(point=>project(point[0],point[1])),polygon=feature.geometryType.includes("Polygon");if(feature.kind==="road"){ctx.strokeStyle="#e3c56b";ctx.lineWidth=2;ctx.beginPath();footprint.forEach((point,i)=>i?ctx.lineTo(...point):ctx.moveTo(...point));ctx.stroke();continue;}if(feature.kind==="water"){ctx.fillStyle="rgba(35,125,176,.74)";ctx.strokeStyle="#55b7df";ctx.lineWidth=1.5;ctx.beginPath();footprint.forEach((point,i)=>i?ctx.lineTo(...point):ctx.moveTo(...point));if(polygon){ctx.closePath();ctx.fill();}ctx.stroke();continue;}if(feature.kind==="green"){ctx.fillStyle="rgba(79,123,85,.66)";ctx.beginPath();footprint.forEach((point,i)=>i?ctx.lineTo(...point):ctx.moveTo(...point));ctx.closePath();ctx.fill();continue;}ctx.fillStyle="rgba(189,203,211,.66)";ctx.strokeStyle="rgba(226,240,247,.82)";ctx.lineWidth=1;ctx.beginPath();footprint.forEach((point,i)=>i?ctx.lineTo(...point):ctx.moveTo(...point));ctx.closePath();ctx.fill();ctx.stroke();}}
    const forward=new THREE.Vector3(-1,0,0).applyQuaternion(airframe.quaternion),airBearing=THREE.MathUtils.radToDeg(Math.atan2(forward.x,forward.y)),rel=(airBearing-miniBearing)*Math.PI/180,cx=w/2,cy=baseY;ctx.save();ctx.translate(cx,cy);ctx.rotate(rel);ctx.fillStyle="#ff5c76";ctx.strokeStyle="#ffffff";ctx.lineWidth=1.4;ctx.beginPath();ctx.moveTo(0,-9);ctx.lineTo(6,7);ctx.lineTo(0,4);ctx.lineTo(-6,7);ctx.closePath();ctx.fill();ctx.stroke();ctx.restore();ctx.fillStyle="#d8f7ff";ctx.font="800 12px system-ui";ctx.fillText(Math.abs(miniBearing)<.01?"N↑":"VIEW↑",7,15);ctx.fillStyle="#9bc5d8";ctx.font="700 9px ui-monospace,monospace";ctx.fillText(`${Math.round(position.z)}m`,7,h-7);if(viewport){viewport.dataset.worldMinimapMode=Math.abs(miniBearing)<.01?"north":"look";viewport.dataset.worldMinimapBearing=miniBearing.toFixed(2);viewport.dataset.worldMinimapFov=this.viewFovDeg.toFixed(1);viewport.dataset.worldMinimapProjection="topdown";viewport.dataset.worldMinimapPitch="0.00";viewport.dataset.worldMinimapRoll="0.00";viewport.dataset.worldMinimapHeightMode="flat-footprints";viewport.dataset.worldMinimapAxisLock=this.minimapAxisLocked?"1":"0";viewport.dataset.worldMinimapAxisLockApplied=!document.fullscreenElement&&this.minimapAxisLocked?"1":"0";viewport.dataset.worldMinimapZoom=(expanded?this.minimapZoom:1).toFixed(2);}
  }
  ...
  async createMap(longitude,latitude){
    ...
    this.mapPixelRatio=Math.min(devicePixelRatio||1,globalThis.__arondightDiagnostics?.presentationSoftwareRaster?WORLD_MAP_SOFTWARE_PIXEL_RATIO:WORLD_MAP_PIXEL_RATIO);
