...
class RealWorldBridge{
  ...
  applyVsCombat(packet){
    ...
    if(packet.type==="hit"){
      ...
      if(killed){const airframe=this.airframeFor(this.threeScene),deathPosition=[Number(airframe?.position?.x)||0,Number(airframe?.position?.y)||0,Number(airframe?.position?.z)||0],baseSeed=(Date.now()^this.vsCombatSeq^Math.round(deathPosition[0]*31)^Math.round(deathPosition[1]*131))>>>0;clearTimeout(this.vsRespawnTimer);const attemptRespawn=attempt=>{if(!this.vsSession)return;const spawn=this.relocateSafeSpawn({mode:"respawn",around:deathPosition,seed:(baseSeed+attempt*0x9e3779b9)>>>0});if(!spawn){if(attempt<5)this.vsRespawnTimer=setTimeout(()=>attemptRespawn(attempt+1),600);return;}this.vsLocalDead=false;this.vsLocalHealth=100;this.updateVsCombatHud(true);this.vsSession.sendCombat({type:"respawn",hp:100,p:[spawn.x,spawn.y,spawn.z],offsetM:spawn.offsetM});};this.vsRespawnTimer=setTimeout(()=>attemptRespawn(0),2200);}return;
    }
  ...
  fail(error){this.loading=false;this.active=false;this.clearBuildingCollisions();this.clearTerrainPhysics();this.terrainOriginElevationM=null;this.buildingVisualLayer?.setVisible?.(false);this.status(`REAL WORLD unavailable · ${error?.message||error}`,"bad");}
  ...
  async activate(locationFix=null){
    ...
    try{
      ...
      await this.createMap(longitude,latitude);await this.initializeTerrainOrigin();this.active=true;
  ...
  syncMapCamera(camera,frameSerial=null){
    ...
    const minimumDistance=fpvTargetDistanceMeters(this.originLat,height,verticalFov,WORLD_MAP_MAX_ZOOM);let focusDistance=minimumDistance;const terrainBelow=terrainHeightAt(this.terrainSnapshot,p.x,p.y);if(!fpv&&dir.z<-.02&&Number.isFinite(terrainBelow)&&p.z>terrainBelow){const ground=(terrainBelow-p.z)/dir.z;if(Number.isFinite(ground)&&ground>0)focusDistance=Math.max(minimumDistance,clamp(ground,2,250));}
