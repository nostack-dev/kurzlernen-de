...
const WORLD_TERRAIN_SOURCE_ID="arondight45-world-dem-presentation";
const WORLD_TERRAIN_TILE_URL="https://tiles.mapterhorn.com/{z}/{x}/{y}.webp";
const WORLD_TERRAIN_ATTRIBUTION="Terrain © Mapterhorn contributors";
const WORLD_TERRAIN_ZOOM=14;
const WORLD_TERRAIN_TILE_SIZE=512;
...
class RealWorldBridge{
  ...
  installUi(){
    ...
    const card=document.createElement("div");card.className="card real-world-card";card.innerHTML=`
      ...
      <div id="realWorldConfig" hidden>
        ...
        <div class="help">Esri World Imagery renders real aerial/satellite pixels on MapLibre GPU terrain; OpenFreeMap + OpenStreetMap add roads and 3D building footprints. Terrain elevation is presentation-only. Loaded OSM building footprints and heights remain bounded static Box3D collision geometry; motor, NAV, AGL and flight-control authority are unchanged.</div>
      </div>
    ...
    mode.value="training";
    const viewport=$("viewport");if(viewport){viewport.dataset.worldMode="training";delete viewport.dataset.worldProvider;delete viewport.dataset.worldRenderPath;delete viewport.dataset.worldLatitude;delete viewport.dataset.worldLongitude;delete viewport.dataset.worldCameraMode;delete viewport.dataset.worldMapCenter;delete viewport.dataset.worldMapZoom;delete viewport.dataset.worldMapPitch;delete viewport.dataset.worldMapBearing;delete viewport.dataset.worldMapSyncMode;delete viewport.dataset.worldMapTargetElevation;delete viewport.dataset.worldThreeFrames;delete viewport.dataset.worldMapUpdates;delete viewport.dataset.worldMapFpsCap;delete viewport.dataset.worldMapPixelRatio;delete viewport.dataset.worldFlightPixelRatio;delete viewport.dataset.worldSymbolsRemoved;delete viewport.dataset.worldLookYaw;delete viewport.dataset.worldLookPitch;delete viewport.dataset.worldLookKeepEnabled;delete viewport.dataset.worldGridEnabled;delete viewport.dataset.worldImageryEnabled;delete viewport.dataset.worldImageryLayer;delete viewport.dataset.worldFlightFps;delete viewport.dataset.worldPerfMode;delete viewport.dataset.worldPaletteLayers;delete viewport.dataset.worldMinimapMode;delete viewport.dataset.worldMinimapBearing;delete viewport.dataset.worldMinimapProjection;delete viewport.dataset.worldMinimapPitch;delete viewport.dataset.worldMinimapRoll;delete viewport.dataset.worldMinimapHeightMode;delete viewport.dataset.worldMinimapAxisLock;delete viewport.dataset.worldMinimapAxisLockApplied;delete viewport.dataset.worldMinimapFov;delete viewport.dataset.worldMinimapZoom;delete viewport.dataset.worldMinimapFeatures;delete viewport.dataset.worldMinimapQueries;delete viewport.dataset.worldMinimapImageryTiles;delete viewport.dataset.worldMinimapImageryZoom;delete viewport.dataset.worldShotQueries;delete viewport.dataset.worldBuildingCollisionStatus;delete viewport.dataset.worldBuildingCollisionFootprints;delete viewport.dataset.worldBuildingCollisionPrisms;delete viewport.dataset.worldBuildingCollisionRevision;delete viewport.dataset.worldTerrainPresentation;delete viewport.dataset.worldTerrainOriginElevationM;}
  ...
  addTerrainPresentation(){
    if(!this.map)return false;
    try{
      if(!this.map.getSource(WORLD_TERRAIN_SOURCE_ID))this.map.addSource(WORLD_TERRAIN_SOURCE_ID,{type:"raster-dem",tiles:[WORLD_TERRAIN_TILE_URL],encoding:"terrarium",tileSize:WORLD_TERRAIN_TILE_SIZE,maxzoom:WORLD_TERRAIN_ZOOM,attribution:WORLD_TERRAIN_ATTRIBUTION});
      this.map.setTerrain({source:WORLD_TERRAIN_SOURCE_ID,exaggeration:1});this.terrainPresentationActive=true;const viewport=$("viewport");if(viewport)viewport.dataset.worldTerrainPresentation="maplibre-gpu-only";return true;
    }catch(error){this.terrainPresentationActive=false;console.warn("WORLD visual terrain unavailable; keeping flat presentation:",error);return false;}
  }
  refreshTerrainOriginElevation(){
    if(!this.terrainPresentationActive||!this.map||!Number.isFinite(this.originLon)||!Number.isFinite(this.originLat)||typeof this.map.queryTerrainElevation!=="function")return this.terrainOriginElevationM;
    try{const elevation=this.map.queryTerrainElevation(new LngLat(this.originLon,this.originLat));if(Number.isFinite(elevation)){this.terrainOriginElevationM=elevation;const viewport=$("viewport");if(viewport)viewport.dataset.worldTerrainOriginElevationM=elevation.toFixed(3);}}catch(error){console.warn("WORLD visual terrain origin query warning:",error);}return this.terrainOriginElevationM;
  }
  ...
  async createMap(longitude,latitude){
    if(this.map){this.geoContainer.hidden=false;this.map.resize();this.map.jumpTo({center:[longitude,latitude],zoom:19,pitch:55,bearing:0});this.terrainOriginElevationM=null;this.setImageryEnabled(this.imageryEnabled);this.addTerrainPresentation();this.addBuildings();this.buildingCollisionDirty=true;return this.map;}
    ...
    const attribution=document.createElement("div");attribution.className="geo-attribution";attribution.textContent="Imagery © Esri, Vantor, Earthstar Geographics, GIS User Community · Terrain © Mapterhorn contributors · Map © OpenFreeMap, OpenMapTiles, OpenStreetMap contributors";container.appendChild(attribution);
    ...
    this.applyFlightPalette();this.stripFlightClutter();this.addWorldImagery();this.addTerrainPresentation();
  ...
  async activate(locationFix=null){
    ...
    try{
      ...
      this.vsWorldFromMate=fromMate;this.originLat=latitude;this.originLon=longitude;this.terrainOriginElevationM=null;const source=fromMate?"MATE GPS ORIGIN":"GPS";this.status(`${source} ${latitude.toFixed(6)}, ${longitude.toFixed(6)} · ±${Math.round(accuracy||0)} m · loading aerial imagery + OSM…`,"warn");
  ...
  syncMapCamera(camera,frameSerial=null){
    ...
    if(!Number.isFinite(this.terrainOriginElevationM))this.refreshTerrainOriginElevation();
    if(typeof this.map.calculateCameraOptionsFromTo!=="function")throw Error("MapLibre eye/target camera API unavailable");const eye=metersToLngLat(this.originLon,this.originLat,p.x,p.y),datum=Number.isFinite(this.terrainOriginElevationM)?this.terrainOriginElevationM:0,eyeElevation=datum+p.z,targetElevation=datum+target.z,options=this.map.calculateCameraOptionsFromTo(new LngLat(eye[0],eye[1]),eyeElevation,new LngLat(center[0],center[1]),targetElevation),zoom=Number(options.zoom),view={...options,center,elevation:targetElevation,roll:clamp(roll,-85,85)};viewport.dataset.worldMapEye=`${eye[0].toFixed(7)},${eye[1].toFixed(7)}`;viewport.dataset.worldMapEyeElevation=p.z.toFixed(3);
