...
let installed=false,pointer=null,surface=null,lastX=0,lastY=0,stickKnob=null,stickEl=null,stickX=0,stickY=0,aimYaw=0,aimPitch=0,stickFrameMs=performance.now(),lastInputMs=performance.now();
...
export function installFootLookCapture(){
  ...
  window.addEventListener("pointerdown",event=>{
    ...
    else{const v=viewport();if(v)v.dataset.walkAimTouch="profiled-dynamic-drag-v12";}
    consume(event);
  ...
  window.addEventListener("pointermove",event=>{
    ...
    if(event.pointerId!==pointer)return;if(surface==="stick")updateStick(event.clientX,event.clientY);else{const dx=event.clientX-lastX,dy=event.clientY-lastY;lastX=event.clientX;lastY=event.clientY;applyDelta(dx,dy,{now:event.timeStamp});}consume(event);
  },{capture:true,passive:false});
  const release=event=>{if(event.pointerId!==pointer)return;const releasedSurface=surface;pointer=null;surface=null;walk()?.endTouchLook?.(releasedSurface==="stick"?"touch-stick":"touch-drag");resetStick();consume(event);};
