...
function fireScreen(clientX,clientY){
  ...
  window.dispatchEvent(new CustomEvent("arondight:foot-screen-fire",{detail:{clientX,clientY,source:"touch-screen-pointerdown"}}));const fired=Boolean(api.fireAt({clientX,clientY,source:"touch-screen-pointerdown"}));const v=viewport();if(v){v.dataset.walkTouchTapRoute="authoritative-screen-pointerdown-v3";v.dataset.walkTouchFire="screen-point-raycast-v3";v.dataset.walkTouchFireResult=fired?"fired":"gated";}return fired;
}
...
export function installFootLookCapture(){
  ...
  window.addEventListener("pointerdown",event=>{
    ...
    if(surface==="stick"){stickEl=captureEl||document.getElementById("footLook");stickKnob=stickEl?.querySelector(".knob")||null;stickFrameMs=performance.now();updateStick(event.clientX,event.clientY);const v=viewport();if(v){v.dataset.walkAimStickCapture="rate-v4";v.dataset.walkAimStickMode="rate-edge-hold-v3";}}
    else{fireScreen(event.clientX,event.clientY);const v=viewport();if(v)v.dataset.walkAimTouch="screen-fire-pointerdown+drag-look-v3";}
    consume(event);
  ...
  const v=viewport();if(v){v.dataset.walkAimCapture="fps-authoritative-v13";v.dataset.walkAimStickMode="rate-edge-hold-v3";v.dataset.walkAimProfile="mouse-lock+screen-fire+radial-rate-stick-v14";v.dataset.walkAimStickCurve="radial-precision-centre-fast-edge-v2";v.dataset.walkTouchTapRoute="authoritative-screen-pointerdown-v3";}
