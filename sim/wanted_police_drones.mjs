...
import {wantedCrimeSeverity,wantedDetectionRadiusM,wantedEscapeDurationMs,wantedLineBlockedByPrisms,wantedPointInRing,wantedPoliceCount,wantedSearchState,wantedStarsForHeat} from "./wanted_system_logic.mjs";
...
const PLAYER_DAMAGE_COOLDOWN_MS=620;
...
function updateTelemetry(remainingMs=0){
  const view=viewport();
  if(!view)return;
  view.dataset.wantedSystem="heat+search+police-drones-v1";
...
function renderHud(remainingMs=0){
  ...
  else{hud.dataset.phase="pursuit";title.textContent="POLICE PURSUIT";detail.textContent=`${active} DRONES · BREAK LINE OF SIGHT`;hud.setAttribute("aria-label",`${stars} wanted stars. ${active} police drones pursuing.`);}
...
function makeMaterials(){
  return{
    body:new THREE.MeshStandardMaterial({color:0x245984,roughness:.34,metalness:.56,emissive:0x061c2d,emissiveIntensity:.28}),
    white:new THREE.MeshStandardMaterial({color:0xe8f1f5,roughness:.42,metalness:.32}),
    dark:new THREE.MeshStandardMaterial({color:0x111820,roughness:.64,metalness:.44}),
...
function ensureSharedVisuals(){
  ...
  geometries??={
    body:new THREE.BoxGeometry(.72,.46,.25),canopy:new THREE.SphereGeometry(.25,9,6),armX:new THREE.BoxGeometry(1.28,.065,.065),armY:new THREE.BoxGeometry(.065,1.28,.065),rotor:new THREE.TorusGeometry(.23,.027,5,12),light:new THREE.SphereGeometry(.07,7,5),gun:new THREE.CylinderGeometry(.035,.045,.30,7),hitbox:new THREE.BoxGeometry(1.58,1.58,.68),tracer:new THREE.CylinderGeometry(.018,.018,1,5),flash:new THREE.BoxGeometry(.76,.50,.27),
  };
}
...
function createDrone(index){
  ...
  const body=visualMesh(geometries.body,materials.body),canopy=visualMesh(geometries.canopy,materials.white),armX=visualMesh(geometries.armX,materials.dark),armY=visualMesh(geometries.armY,materials.dark),gun=visualMesh(geometries.gun,materials.dark),flash=visualMesh(geometries.flash,materials.flash);
  ...
  const red=visualMesh(geometries.light,materials.red),blue=visualMesh(geometries.light,materials.blue);red.position.set(.18,-.26,.17);blue.position.set(.18,.26,.17);
  ...
  root.add(body,canopy,armX,armY,gun,red,blue,flash,hitbox);policeRoot.add(root);
  ...
  return{index,root,body,rotors,red,blue,flash,hitbox,tracer,explosion:createExplosion(index),velocity:new THREE.Vector3(),goal:new THREE.Vector3(),active:false,hp:POLICE_HP,destroyedUntil:0,spawnSerial:0,nextShotAt:0,tracerUntil:0,hitUntil:0,nextSensorAt:0,nextGoalRoofAt:0,nextCollisionAt:0,goalRoof:0,collisionRoof:0,seesPlayer:false,distance:Infinity};
}

function ensureScene(){
  const scene=bridge()?.threeScene;if(!scene)return false;
  if(scene===sceneRef&&policeRoot)return true;
  policeRoot?.parent?.remove(policeRoot);sceneRef=scene;policeRoot=new THREE.Group();policeRoot.name="WANTED_POLICE_DRONES";scene.add(policeRoot);drones.splice(0,drones.length);for(let index=0;index<MAX_POLICE_DRONES;index++)drones.push(createDrone(index));return true;
}
...
function spawnDrone(drone,player,now){
  const angle=drone.index/MAX_POLICE_DRONES*Math.PI*2+now*.00017,radius=27+(drone.index%3)*5,x=player.x+Math.cos(angle)*radius,y=player.y+Math.sin(angle)*radius,z=safeAltitude(x,y,player.z+5+(drone.index%2)*1.8);
  drone.root.position.set(x,y,z);drone.root.rotation.set(0,0,angle+Math.PI);drone.root.scale.setScalar(1);drone.velocity.set(-Math.sin(angle)*1.4,Math.cos(angle)*1.4,0);drone.hp=POLICE_HP;drone.active=true;drone.root.visible=true;drone.hitbox.visible=true;drone.flash.visible=false;drone.hitUntil=0;drone.spawnSerial++;drone.nextShotAt=now+900+drone.index*190;drone.nextSensorAt=now+drone.index*24;drone.nextGoalRoofAt=0;drone.nextCollisionAt=0;drone.goalRoof=0;drone.collisionRoof=0;drone.seesPlayer=true;drone.distance=drone.root.position.distanceTo(player);drone.tracer.visible=false;
}

function deactivateDrone(drone){drone.active=false;drone.root.visible=false;drone.hitbox.visible=false;drone.tracer.visible=false;drone.seesPlayer=false;drone.distance=Infinity;}
...
function registerPoliceHit(hit){
  ...
  const killed=drone.hp===0;window.dispatchEvent(new CustomEvent("arondight:combat-hit-confirm",{detail:{police:true,damage:POLICE_HIT_DAMAGE,hp:drone.hp,killed}}));
  ...
  if(killed){const crimeId=`police:${drone.index}:${drone.spawnSerial}`;explodeDrone(drone,now);deactivateDrone(drone);drone.destroyedUntil=now+POLICE_RESPAWN_MS;policeKills++;reportCrime({id:crimeId,kind:"police-drone"});}
  return true;
}
...
function updateDroneMotion(drone,now,dt){
  const goal=droneGoal(drone,now),distance=drone.root.position.distanceTo(goal),maxSpeed=(phase==="searching"?6.4:8.4+stars*.72)+(distance>45?3.8:0),speed=Math.min(maxSpeed,Math.max(1.8,distance*.72));
  desiredVelocity.copy(goal).sub(drone.root.position);if(desiredVelocity.lengthSq()>1e-6)desiredVelocity.normalize().multiplyScalar(speed);const response=1-Math.exp(-3.4*dt);drone.velocity.lerp(desiredVelocity,response);proposedPosition.copy(drone.root.position).addScaledVector(drone.velocity,dt);
  if(now>=drone.nextCollisionAt){drone.nextCollisionAt=now+90+drone.index*6;drone.collisionRoof=buildingTopAt(proposedPosition.x,proposedPosition.y);}const roof=drone.collisionRoof;if(roof&&proposedPosition.z<roof+1.15){proposedPosition.x=drone.root.position.x;proposedPosition.y=drone.root.position.y;proposedPosition.z=Math.min(roof+1.7,drone.root.position.z+Math.max(2.5,roof-drone.root.position.z+1)*dt);drone.velocity.x*=.45;drone.velocity.y*=.45;drone.velocity.z=Math.max(2.5,drone.velocity.z);}
  drone.root.position.copy(proposedPosition);if(Math.hypot(drone.velocity.x,drone.velocity.y)>.08)drone.root.rotation.z=Math.atan2(drone.velocity.y,drone.velocity.x);const rotorAngle=now*.031+drone.index;for(const rotor of drone.rotors)rotor.rotation.z=rotorAngle;const pulse=((Math.floor(now/145)+drone.index)&1)===0;drone.red.visible=pulse;drone.blue.visible=!pulse;if(now>=drone.hitUntil)drone.flash.visible=false;else drone.root.scale.setScalar(1.08);if(now>=drone.hitUntil)drone.root.scale.lerp(unitScale,Math.min(1,dt*12));if(now>=drone.tracerUntil)drone.tracer.visible=false;
}
...
function applyPlayerDamage(drone,now){
  if(now-lastPlayerDamageAt<PLAYER_DAMAGE_COOLDOWN_MS||currentPlayerHp()<=0)return false;lastPlayerDamageAt=now;const amount=Math.min(11,6+stars),before=currentPlayerHp();
  ...
  const view=viewport();if(view){view.dataset.wantedPoliceDamage=String((Number(view.dataset.wantedPoliceDamage)||0)+amount);view.dataset.wantedPoliceShotsHit=String((Number(view.dataset.wantedPoliceShotsHit)||0)+1);}return true;
}

function updateDroneAttack(drone,player,now){
  if(phase!=="pursuit"||!drone.seesPlayer||drone.distance<4.5||drone.distance>34+stars*2||now<drone.nextShotAt||currentPlayerHp()<=0)return;
  drone.nextShotAt=now+Math.max(780,1850-stars*150+drone.index*57);tmpPosition.copy(player);tmpPosition.x+=(drone.index%2?.18:-.18);tmpPosition.z+=.05;showTracer(drone,drone.root.position,tmpPosition,now);playTone({frequency:310,endFrequency:145,duration:.075,gain:.015,type:"square"});applyPlayerDamage(drone,now);
...
export function installWantedPoliceDrones(){
  ...
  addEventListener(WORLD_KILL_EVENT,onWorldKill);addEventListener("arondight:combat-hit-confirm",onCombatKill);addEventListener(AUDIO_SETTINGS_EVENT,event=>{audioSettings=normalizeAudioSettings(event.detail||loadAudioSettings());});
