...
function updateDroneAttack(drone,now){
  ...
  showTracer(drone,drone.root.position,tmpPosition,now);playTone({frequency:310,endFrequency:145,duration:.075,gain:.015,type:"square"});window.dispatchEvent(new CustomEvent("arondight:world-gunshot",{detail:{position:[drone.root.position.x,drone.root.position.y,drone.root.position.z],end:[tmpPosition.x,tmpPosition.y,tmpPosition.z],source:"police-drone",weapon:"police"}}));const damaged=accurate&&applyTargetDamage(drone,target,now),view=viewport();if(view){view.dataset.wantedPoliceShots=String((Number(view.dataset.wantedPoliceShots)||0)+1);if(!accurate)view.dataset.wantedPoliceShotsMissed=String((Number(view.dataset.wantedPoliceShotsMissed)||0)+1);view.dataset.wantedPoliceLastShotResult=accurate?(damaged?"hit":"grace"):"miss";view.dataset.wantedPoliceLastHitChance=chance.toFixed(3);view.dataset.wantedPoliceLastShotRoll=roll.toFixed(3);view.dataset.wantedPoliceShotTarget=target.kind;}
}
