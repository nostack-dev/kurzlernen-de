...
let installed=false,boundScene=null,lifeRoot=null,lightRoot=null,treeRoot=null,lampRoot=null,lastRouteRefresh=-Infinity,lastOriginKey="",lastFrame=performance.now(),audioCtx=null,audioUnlocked=false,lastChirp=0,wrappedHit=null,particlePoints=null,particleGeometry=null,particlePositions=null,particleColors=null,particleCursor=0,treeTrunks=null,treeCrowns=null,lampPoles=null,lampHeads=null,mapStyledFor=null,forcedDefaultOff=false,worldVisibleLatched=false;
...
function worldVisible(){const v=viewport(),real=v?.dataset?.worldMode==="real";if(!real){worldVisibleLatched=false;return false;}if(bridge()?.active)worldVisibleLatched=true;return worldVisibleLatched;}
...
function makeCar(index){
  ...
  const record={id,kind:"car",index,group,seed,speed:8+(seed%55)/10,phase:(seed%10000)/10000,laneSign:seed&1?1:-1,side:0,deadUntil:0,dyingUntil:0,color:mat.color.getHex(),routeKey:""};
...
function makeBus(index){
  ...
  const record={id,kind:"bus",index,group,seed,speed:5.2+(seed%28)/10,phase:(seed%9000)/9000,laneSign:seed&1?1:-1,side:0,deadUntil:0,dyingUntil:0,color:mat.color.getHex(),routeKey:""};
...
function makePerson(index){
  ...
  const record={id,kind:"person",index,group,seed,speed:1.0+(seed%65)/100,phase:(seed%8000)/8000,laneSign:1,side:(seed&1?1:-1)*(2.8+(seed%15)/20),deadUntil:0,dyingUntil:0,color:shirt.color.getHex(),routeKey:""};
...
function boundRouteKeys(){return new Set(records.filter(record=>record.kind!=="bird").map(record=>record.routeKey).filter(Boolean));}
function refreshRoutes(now){
  ...
  if(key!==lastOriginKey&&routeCache.size){for(const [routeKey,old] of routeCache){const rebuilt=buildTrafficRoute(old.geoPath,{originLon:b.originLon,originLat:b.originLat,roadClass:old.roadClass,lastSeen:old.lastSeen});if(rebuilt)routeCache.set(routeKey,rebuilt);}}
  lastOriginKey=key;
  ...
  const bound=boundRouteKeys();for(const [routeKey,route] of routeCache)if(now-route.lastSeen>ROUTE_DROP_MS&&!bound.has(routeKey))routeCache.delete(routeKey);
  const fresh=[...routeCache.values()].filter(route=>now-route.lastSeen<=ROUTE_STALE_MS),candidatePool=(fresh.length?fresh:[...routeCache.values()]).sort((a,c)=>a.key.localeCompare(c.key)),boundRoutes=[...bound].map(routeKey=>routeCache.get(routeKey)).filter(Boolean),pool=[...new Map([...boundRoutes,...candidatePool].map(route=>[route.key,route])).values()].slice(0,MAX_ROUTE_POOL);
  if(pool.length){routes.splice(0,routes.length,...pool);positionDecor();}
  const v=viewport();if(v){v.dataset.worldLifeRoutes=String(routes.length);v.dataset.worldLifeCachedRoutes=String(routeCache.size);v.dataset.worldLifeRouteGapHeld=seen.size?"0":routes.length?"1":"0";v.dataset.worldLifeStableRoutes=String(bound.size);v.dataset.worldLifeAlive="1";}
}
...
function routeFor(record){if(record.routeKey){const bound=routeCache.get(record.routeKey);if(bound)return bound;}if(!routes.length)return null;const route=routes[(record.index*7+(record.kind==="person"?3:record.kind==="bus"?5:1))%routes.length]||null;if(route)record.routeKey=route.key;return route;}
...
function updateRecord(record,epochSec,now,dt){
  if(record.kind==="bird"){
    ...
    const a=epochSec*(.22+(record.seed%22)/1000)+record.phase*Math.PI*2,r=18+(record.seed%34),cx=((record.seed>>>7)%25)-12,cy=((record.seed>>>13)%25)-12,z=8+(record.seed%13)+Math.sin(a*2.3+record.index)*2.1;record.group.position.set(cx+Math.cos(a)*r,cy+Math.sin(a*.94)*r,z);record.group.rotation.set(.08*Math.sin(a*3),0,a+Math.PI/2);const flap=.82+.20*Math.sin(epochSec*10+record.index);record.group.scale.set(1,flap,1);record.group.userData.worldLifePositioned=true;record.group.visible=true;return;
  }
  if(Date.now()<record.deadUntil){record.group.visible=false;return;}
  const route=routeFor(record);if(!route){record.group.visible=record.group.userData.worldLifePositioned===true;return;}const speed=record.speed,offset=record.kind==="person"?record.side:carLane(route,record),p=sampleRoute(route,record.phase*route.length*2+epochSec*speed,offset);if(!p){record.group.visible=record.group.userData.worldLifePositioned===true;return;}record.group.position.set(p.x,p.y,record.kind==="person"?.01*Math.sin(epochSec*6+record.index):0);record.group.rotation.set(0,0,p.yaw);record.group.userData.worldLifePositioned=true;record.group.visible=true;
}
...
function loop(now=performance.now()){const dt=Math.min(.05,Math.max(0,(now-lastFrame)/1000||0));lastFrame=now;const b=bridge();syncDefaultOff();patchSettingsUi();wrapPopulationHits();if(ensureScene()){const active=worldVisible();lifeRoot.visible=active;lightRoot.visible=active;treeRoot.visible=active;lampRoot.visible=active;if(active){refreshRoutes(now);styleMap();const t=Date.now()/1000;for(const r of records)updateRecord(r,t,now,dt);updateParticles(now,dt);playAmbientBird(now);const v=viewport();if(v){v.dataset.worldLifeVisible=String(records.filter(r=>r.group.visible).length);v.dataset.worldLifeTotal=String(records.length);v.dataset.worldLifeTotalApproxCars=String((MOBILE?8:14)+EXTRA_CARS);v.dataset.worldLifeTotalApproxPeople=String((MOBILE?8:14)+EXTRA_PEOPLE);v.dataset.worldLifeContinuity="latched-routes-v2";}}else{particlePoints.visible=false;}}requestAnimationFrame(loop);}
