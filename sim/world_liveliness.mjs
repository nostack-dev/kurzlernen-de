...
function makeCar(index){ensureShared();const seed=hashText(`life-car:${index}`),id=`life-car-${index}`,group=new THREE.Group(),mesh=new THREE.Mesh(shared.carGeo,shared.carMats[seed%shared.carMats.length]);const record={id,kind:"car",index,group,seed,speed:8+(seed%55)/10,phase:(seed%10000)/10000,laneSign:seed&1?1:-1,side:0,deadUntil:0,dyingUntil:0,color:mesh.material.color.getHex()};mesh.position.z=.46;tag(mesh,record);group.add(mesh);group.userData.worldLifeId=id;group.userData.worldLifeKind="car";group.userData.worldPopulationKind=populationKind(record);return record;}
function makeBus(index){ensureShared();const seed=hashText(`life-bus:${index}`),id=`life-bus-${index}`,group=new THREE.Group(),body=new THREE.Mesh(shared.busGeo,shared.busMats[seed%shared.busMats.length]),windows=new THREE.Mesh(shared.busWindowGeo,shared.busWindow);const record={id,kind:"bus",index,group,seed,speed:5.2+(seed%28)/10,phase:(seed%9000)/9000,laneSign:seed&1?1:-1,side:0,deadUntil:0,dyingUntil:0,color:body.material.color.getHex()};body.position.z=1.30;windows.position.set(.15,0,1.77);tag(body,record);tag(windows,record);group.add(body,windows);group.userData.worldLifeId=id;group.userData.worldLifeKind="bus";group.userData.worldPopulationKind=populationKind(record);return record;}
function makePerson(index){ensureShared();const seed=hashText(`life-person:${index}`),id=`life-person-${index}`,group=new THREE.Group(),body=new THREE.Mesh(shared.personGeo,shared.shirtMats[seed%shared.shirtMats.length]),head=new THREE.Mesh(shared.headGeo,shared.skinMats[(seed>>>3)%shared.skinMats.length]);const record={id,kind:"person",index,group,seed,speed:1.0+(seed%65)/100,phase:(seed%8000)/8000,laneSign:1,side:(seed&1?1:-1)*(2.8+(seed%15)/20),deadUntil:0,dyingUntil:0,color:body.material.color.getHex()};body.position.z=.63;head.position.z=1.31;tag(body,record);tag(head,record);group.add(body,head);group.userData.worldLifeId=id;group.userData.worldLifeKind="person";group.userData.worldPopulationKind=populationKind(record);return record;}
