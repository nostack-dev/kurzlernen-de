...
function locallyOwnsVehicle(id,root=null){const d=drive(),key=String(id||"");return Boolean(key&&((d?.active&&String(d.vehicleId||"")===key)||root?.userData?.playerDriven===true));}
...
function applyRemoteVehicle(peerId,cv){
  peerId=String(peerId||"");if(!peerId||peerId===selfId()||!cv||typeof cv.id!=="string"||!finiteArray(cv.p,3)||!finiteArray(cv.q,4)||!finiteArray(cv.v,3)||!finiteArray(cv.w,3))return false;const id=String(cv.id),position=canonicalToLocal(cv.p),root=rootFor(id);if(!root)return false;
  if(locallyOwnsVehicle(id,root)){
    releaseRemote(peerId,"local-player-authority");
    const v=viewport();if(v){v.dataset.vsRemoteVehicleRejected="local-player-authority-v1";v.dataset.vsRemoteVehicleRejectedId=id;}
    return false;
  }
  const yaw=Number.isFinite(cv.yaw)?Number(cv.yaw):Math.atan2(2*(cv.q[3]*cv.q[2]+cv.q[0]*cv.q[1]),1-2*(cv.q[1]*cv.q[1]+cv.q[2]*cv.q[2]));ensurePhysicsBody(id,root,position,yaw);const p=physics(),current=p?.pose?.(id),owner=`remote-player:${peerId}`,error=current?.position?Math.hypot(current.position[0]-position[0],current.position[1]-position[1],current.position[2]-position[2]):Infinity;
  if(!current||error>SNAP_ERROR_M)p?.setPose?.(id,{position,yaw,velocity:[...cv.v],angularVelocity:[...cv.w],owner,priority:90,leaseMs:REMOTE_LEASE_MS});else{const speed=Math.hypot(cv.v[0],cv.v[1]),ahead=.11,predicted=[position[0]+cv.v[0]*ahead,position[1]+cv.v[1]*ahead,position[2]+cv.v[2]*ahead];p?.setTarget?.(id,{position:predicted,yaw,speedMps:speed,response:7.5,maxAccelerationMps2:18,owner,priority:90,leaseMs:REMOTE_LEASE_MS});}
