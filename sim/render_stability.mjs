...
const APPLE_MOBILE_RE=/(?:iphone|ipad|ipod)|(?:macintosh.*mobile)/i;
...
export function renderPlatformProfile({userAgent="",devicePixelRatio=1,rendererName=""}={}){
  ...
  const nativePixelRatio=Math.max(.25,Number(devicePixelRatio)||1),pixelRatioCeiling=software?Math.min(nativePixelRatio,.30):android?Math.min(nativePixelRatio,.90):appleMobile?Math.min(nativePixelRatio,1):Math.min(nativePixelRatio,1.25);
