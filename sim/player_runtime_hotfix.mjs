...
let patchedRegister=null,shotSeq=0;
const callbackKinds=new WeakMap();
const tmpPos=new THREE.Vector3(),tmpQuat=new THREE.Quaternion(),tmpEuler=new THREE.Euler();

function emptyCache(){return{peer:[],vehicles:[],population:[],decor:[],fire:[],localAvatar:null,generation:0};}
function viewport(){return document.getElementById("viewport");}
function bridge(){return globalThis.__arondightRealWorld||null;}
function walk(){return globalThis.__arondightWalkMode||null;}
function runtime(){return globalThis.__arondightPlayerVehicleRuntime||null;}
...
function pushUnique(list,node){if(node&&!list.includes(node))list.push(node);}
function indexNode(node,target){
  ...
  if(node.isMesh&&!u.arondightAirframe&&!u.flightFireDecal&&!u.flightFireIgnore&&node.material?.visible!==false)target.fire.push(node);
}
...
function startIndex(scene,force=false){
  if(!scene)return;if(build&&!force)return;const now=performance.now();if(!force&&now-lastIndexStart<INDEX_REFRESH_MS)return;lastIndexStart=now;
  build={scene,queue:[scene],next:emptyCache()};pumpIndex();
}
function pumpIndex(){
...
function scheduleReindex(){clearTimeout(indexTimer);indexTimer=setTimeout(()=>{const scene=bridge()?.threeScene;if(scene)startIndex(scene,true);scheduleReindex();},INDEX_REFRESH_MS);}
function patchSceneTraversal(scene){
  ...
  const wrapper=function(callback){
    const kind=classifyTraverse(callback);if(kind==="native")return base(callback);const list=cache[kind]||build?.next?.[kind]||[];
    for(const node of list){if(!node)continue;callback(node);if(kind==="decor"&&node.isMesh&&!node.userData?.flightFireIgnore&&!node.userData?.flightFireDecal)pushUnique(cache.fire,node);}
  };
...
function makeLocalAvatarShootable(scene){
  const root=localAvatarRoot(scene);if(!root)return false;cache.localAvatar=root;const sid=selfId();const stack=[root];let count=0;
  while(stack.length){const node=stack.pop();if(node?.isMesh){node.userData.flightFireIgnore=false;node.userData.worldPopulationClone=false;if(sid&&!node.userData.vsPlayerId)node.userData.vsPlayerId=sid;pushUnique(cache.fire,node);count++;}for(const child of node?.children||[])stack.push(child);}
...
function patchSelfHitRouting(){
  const b=bridge(),current=b?.registerVsHit;if(!b||typeof current!=="function"||current===patchedRegister)return;
  if(!current.__worldActionFeedbackWrapper)return;const base=current.bind(b);const wrapper=hit=>isLocalHumanHit(hit)?sendSelfHit(hit):Boolean(base(hit));
  wrapper.__playerRuntimeSelfHit=true;wrapper.__worldActionFeedbackWrapper=true;wrapper.__worldActionFeedbackBase=current.__worldActionFeedbackBase||current;patchedRegister=wrapper;b.registerVsHit=wrapper;const v=viewport();if(v)v.dataset.selfHitRouting="local-authoritative-v2";
}
...
function installAim(){
  document.addEventListener("pointerdown",event=>{
    const v=viewport(),w=walk();if(w?.mode!=="foot"||!v||!gameplayTarget(event.target))return;
    if(event.pointerType==="mouse"&&event.button!==0)return;
    if(event.pointerType==="mouse"&&document.pointerLockElement===v){w.fire?.();return;}
    dragPointer=event.pointerId;lastPointerX=event.clientX;lastPointerY=event.clientY;
    if(event.pointerType==="mouse"&&v.requestPointerLock)try{v.requestPointerLock();}catch{}
  ...
  const release=event=>{if(event.pointerId===dragPointer)dragPointer=null;};document.addEventListener("pointerup",release,{capture:true,passive:true});document.addEventListener("pointercancel",release,{capture:true,passive:true});
  document.addEventListener("contextmenu",event=>{if(walk()?.mode==="foot"&&gameplayTarget(event.target))event.preventDefault();},{capture:true});
  const v=viewport();if(v)v.dataset.walkAimProfile="pointerlock+drag-center-v6";
}
...
function performPlayerReset(seq){
  if(seq!==resetSeq)return;const w=walk(),r=runtime(),v=viewport();if(!w)return;const pose=resetPoseFromAirframe();if(v)v.dataset.vsManualRespawns=String((Number(v.dataset.vsManualRespawns)||0)+1);
  if(w.mode==="foot"){w.setPose?.(pose);if(v){v.dataset.playerReset="foot+health-v3";v.dataset.selfHp="100";}const b=bridge();if(b&&!selfId()){b.vsLocalHealth=100;b.vsLocalDead=false;b.updateVsCombatHud?.(true);}return;}
  w.setMode?.("foot");requestAnimationFrame(()=>{if(seq!==resetSeq)return;w.setPose?.(pose);requestAnimationFrame(()=>{if(seq!==resetSeq)return;w.setMode?.("drone");requestAnimationFrame(()=>{if(seq!==resetSeq)return;r?.teleportDrone?.(pose);const b=bridge();if(b&&!selfId()){b.vsLocalHealth=100;b.vsLocalDead=false;b.updateVsCombatHud?.(true);}if(v){v.dataset.playerReset="human+drone+health-v3";v.dataset.selfHp="100";}});});});
}
...
function maintenance(now=performance.now()){
  const scene=bridge()?.threeScene;if(scene){patchSceneTraversal(scene);if(now-lastAvatarCheck>250){lastAvatarCheck=now;makeLocalAvatarShootable(scene);}if(!build&&now-lastIndexStart>INDEX_REFRESH_MS)startIndex(scene,true);}
  patchSelfHitRouting();requestAnimationFrame(maintenance);
}
function onWorldChanged(){const scene=bridge()?.threeScene;if(scene)startIndex(scene,true);}

export function installPlayerRuntimeHotfix(){
  if(installed)return;installed=true;installAim();installResetHook();addEventListener(VS_PEER_EVENT,onWorldChanged);addEventListener(VS_POSE_EVENT,onWorldChanged);scheduleReindex();const v=viewport();if(v){v.dataset.playerRuntimeHotfix="reset+selfhit+aim+cached-scene-v3";v.dataset.walkAimProfile="pointerlock+drag-center-v6";}requestAnimationFrame(maintenance);
}
