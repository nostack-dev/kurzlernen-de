...
function publish(reason="active"){
  const view=viewport();if(!view)return;view.dataset.walkInputLifecycleGuard="exclusive-move-pointer-v3";view.dataset.walkMoveStickOwner=activePointer===null?"none":String(activePointer);if(reason)view.dataset.walkMoveStickLifecycle=reason;
}
...
function release(reason="lifecycle"){
  ...
  const view=viewport();if(view){view.dataset.walkMoveStickRecovery=reason;view.dataset.walkMoveStickRecoveries=String((Number(view.dataset.walkMoveStickRecoveries)||0)+1);view.dataset.walkInputLifecycleGuard="exclusive-move-pointer-v3";view.dataset.walkMoveStickOwner="none";}return true;
}
function lostCapture(event){
  if(event.pointerId===activePointer){release("lostpointercapture");return;}
  // A right-stick, fire, weapon or other touch losing capture must never bubble
  // into legacy recovery listeners that zero the currently held move stick.
  event.stopPropagation();const view=viewport();if(view)view.dataset.walkMoveStickForeignCapture="ignored";
}
