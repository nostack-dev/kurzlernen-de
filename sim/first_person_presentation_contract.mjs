...
function syncViewmodelPose({hide=true}={}){
  const b=bridge(),scene=b?.threeScene,w=walk(),gun=scene?.getObjectByName?.("WALK_PISTOL_3D"),view=viewport();if(!gun)return false;
  if(!footActive()||!w?.viewRay){gun.visible=false;return false;}const ray=w.viewRay();if(!finite3(ray?.origin)||!finite3(ray?.direction))return false;
  vmOrigin.set(...ray.origin);vmDirection.set(...ray.direction).normalize();vmCamera.position.copy(vmOrigin);vmCamera.up.set(0,0,1);vmCamera.lookAt(vmTarget.copy(vmOrigin).add(vmDirection));vmCamera.updateMatrixWorld(true);vmWorld.copy(VM_LOCAL_OFFSET);vmCamera.localToWorld(vmWorld);vmQuaternion.copy(vmCamera.quaternion);gun.scale.setScalar(1);setWorldPosition(gun,vmWorld);setWorldQuaternion(gun,vmQuaternion);gun.rotateX(-.045);gun.rotateY(-.065);gun.rotateZ(-.018);gun.updateMatrixWorld?.(true);if(hide)gun.visible=false;
  const flash=gun.getObjectByName?.("FINAL_MUZZLE_FLASH")||gun.getObjectByName?.("WALK_MUZZLE_FLASH"),muzzle=new THREE.Vector3();flash?.getWorldPosition?.(muzzle);if(view){view.dataset.walkViewmodelOwnership="fp-render-only+current-pose-v1";view.dataset.walkViewmodelAnchor="current-walk-view-ray-v1";view.dataset.walkViewmodelOrigin=`${vmOrigin.x.toFixed(3)},${vmOrigin.y.toFixed(3)},${vmOrigin.z.toFixed(3)}`;if(flash)view.dataset.walkMuzzleWorld=`${muzzle.x.toFixed(3)},${muzzle.y.toFixed(3)},${muzzle.z.toFixed(3)}`;}return true;
}
function patchFootWeaponFire(){
  const api=footWeapons();if(!api||api===patchedFootWeapons||typeof api.fireAt!=="function")return false;baseFireAt=api.fireAt.bind(api);api.fireAt=args=>{syncViewmodelPose({hide:true});return baseFireAt(args);};api.__firstPersonPresentationFireSync=true;patchedFootWeapons=api;const view=viewport();if(view)view.dataset.walkMuzzleShotSync="pre-fire-current-pose-v1";return true;
}
function patchRenderFrame(){
  ...
  baseRenderFrame=b.renderFrame.bind(b);const wrapper=function(renderer,scene,camera){const fp=footActive(),rig=ensureLocalRig(scene);if(fp&&rig)rig.group.visible=false;try{return baseRenderFrame(renderer,scene,camera);}finally{const gun=scene?.getObjectByName?.("WALK_PISTOL_3D");if(gun)gun.visible=false;if(fp&&rig&&!walk()?.dead)rig.group.visible=true;const view=viewport();if(view)view.dataset.walkRenderOwnership="world-avatar+render-only-viewmodel-v1";}};wrapper.__firstPersonPresentationContract=true;b.renderFrame=wrapper;patchedBridge=b;return true;
}
function frame(now=performance.now()){patchRenderFrame();patchFootWeaponFire();syncLocalRig(now);syncViewmodelPose({hide:true});requestAnimationFrame(frame);}

export function installFirstPersonPresentationContract(){
  if(installed)return;installed=true;addEventListener("arondight:foot-screen-fire",()=>syncViewmodelPose({hide:true}),{capture:true});addEventListener("arondight:player-mode",()=>{syncLocalRig();syncViewmodelPose({hide:true});});const view=viewport();if(view){view.dataset.firstPersonPresentationContract="viewmodel-vs-world-avatar-v1";view.dataset.walkViewmodelOwnership="fp-render-only+current-pose-v1";}requestAnimationFrame(frame);
}
