...
function presentViewmodel({camera,scene}={}){
  ...
  scene.updateMatrixWorld?.(true);camera.updateMatrixWorld?.(true);gun.updateMatrixWorld?.(true);if(gun.parent!==camera)camera.attach(gun);lastBaseLocalPosition.copy(gun.position);lastBaseLocalQuaternion.copy(gun.quaternion);hasBaseLocal=true;lastRenderCameraPosition.copy(camera.position);lastRenderCameraQuaternion.copy(camera.quaternion);lastRenderFov=Number(camera.fov)||78;lastRenderAspect=Number(camera.aspect)||16/9;hasRenderCamera=true;
  let error=NaN,ray=null;if(screenAimActive){ray=canonicalScreenRay(screenX,screenY);if(ray)error=alignWeaponToRay(gun,ray);}gun.updateMatrixWorld?.(true);const flash=gun.getObjectByName?.("FINAL_MUZZLE_FLASH")||gun.getObjectByName?.("WALK_MUZZLE_FLASH");flash?.getWorldPosition?.(muzzleWorld);const view=viewport();if(view){view.dataset.firstPersonPresentationContract="camera-child-screen-ray-v3";view.dataset.walkViewmodelOwnership="single-final-render-fire-owner-v3";view.dataset.walkViewmodelRenderParent="camera";view.dataset.walkViewmodelDetachPolicy="post-render-scene";view.dataset.walkViewmodelCameraFrames=String((Number(view.dataset.walkViewmodelCameraFrames)||0)+1);view.dataset.walkMuzzleWorld=`${muzzleWorld.x.toFixed(3)},${muzzleWorld.y.toFixed(3)},${muzzleWorld.z.toFixed(3)}`;}if(ray)publishAlignment(ray,error,"render");return true;
}
...
export function installFirstPersonPresentationContract(){
  if(installed)return;installed=true;suppressLegacyWeaponTransformOwner();addEventListener("arondight:foot-screen-aim",onScreenAim,{capture:true});addEventListener("arondight:foot-screen-fire",onScreenFire,{capture:true});addEventListener("arondight:player-mode",()=>{screenAimActive=false;syncLocalRig();});const view=viewport();if(view){view.dataset.firstPersonPresentationContract="camera-child-screen-ray-v3";view.dataset.walkViewmodelOwnership="single-final-render-fire-owner-v3";view.dataset.walkViewmodelVisibilityOwner="player-walk-mode-v4";}requestAnimationFrame(frame);
}
