...
export function installFlightFireFx({viewport,scene,camera,worldBridge=null,isEnabled=()=>document.body.classList.contains("solo-flight"),isPointerEnabled=()=>true,onRecoil=()=>{}}={}){
  ...
  viewport.dataset.fireCombatLocked="0";
  ...
  function ensureAudio(){if(audioCtx)return audioCtx;const Ctx=window.AudioContext||window.webkitAudioContext;if(!Ctx)return null;audioCtx=new Ctx();audioMaster=audioCtx.createGain();audioMaster.gain.value=audioSettings.soundEnabled?1:0;audioMaster.connect(audioCtx.destination);noiseBuffer=audioCtx.createBuffer(1,Math.floor(audioCtx.sampleRate*.045),audioCtx.sampleRate);const data=noiseBuffer.getChannelData(0);for(let i=0;i<data.length;i++)data[i]=(Math.random()*2-1)*(1-i/data.length);return audioCtx;}
  function shotSound(){if(!audioSettings.soundEnabled||audioSettings.shotsVolume<=0)return;const level=audioSettings.shotsVolume/100,ctx=ensureAudio();if(!ctx||!noiseBuffer||!audioMaster)return;try{if(ctx.state==="suspended")ctx.resume();const t=ctx.currentTime,src=ctx.createBufferSource(),filter=ctx.createBiquadFilter(),gain=ctx.createGain(),thump=ctx.createOscillator(),tg=ctx.createGain();src.buffer=noiseBuffer;filter.type="bandpass";filter.frequency.value=1650;filter.Q.value=.58;gain.gain.setValueAtTime(.14*level,t);gain.gain.exponentialRampToValueAtTime(.001,t+.052);thump.type="triangle";thump.frequency.setValueAtTime(155,t);thump.frequency.exponentialRampToValueAtTime(78,t+.06);tg.gain.setValueAtTime(.07*level,t);tg.gain.exponentialRampToValueAtTime(.001,t+.065);src.connect(filter).connect(gain).connect(audioMaster);thump.connect(tg).connect(audioMaster);src.start(t);src.stop(t+.06);thump.start(t);thump.stop(t+.07);}catch{}}
  function hitConfirmSound(){if(!audioSettings.soundEnabled||audioSettings.fxVolume<=0)return;const ctx=ensureAudio();if(!ctx||!audioMaster)return;try{const level=audioSettings.fxVolume/100,t=ctx.currentTime,osc=ctx.createOscillator(),gain=ctx.createGain();osc.frequency.setValueAtTime(900,t);osc.frequency.exponentialRampToValueAtTime(1500,t+.05);gain.gain.setValueAtTime(.055*level,t);gain.gain.exponentialRampToValueAtTime(.001,t+.07);osc.connect(gain).connect(audioMaster);osc.start(t);osc.stop(t+.075);}catch{}}
  function damageSound(){if(!audioSettings.soundEnabled||audioSettings.fxVolume<=0)return;const ctx=ensureAudio();if(!ctx||!audioMaster)return;try{const level=audioSettings.fxVolume/100,t=ctx.currentTime,osc=ctx.createOscillator(),gain=ctx.createGain();osc.type="triangle";osc.frequency.setValueAtTime(125,t);osc.frequency.exponentialRampToValueAtTime(72,t+.12);gain.gain.setValueAtTime(.05*level,t);gain.gain.exponentialRampToValueAtTime(.001,t+.14);osc.connect(gain).connect(audioMaster);osc.start(t);osc.stop(t+.15);}catch{}}
  ...
  function setGamepadFire(pressed){if(!pressed||!isEnabled()){if(active?.source==="gamepad")stop();return false;}if(active&&active.source!=="gamepad")return false;if(!active){active={id:"xbox",source:"gamepad"};ensureAudio();nextShotAt=0;}fire(performance.now());scheduleFire();return true;}
  ...
  return{stop,setGamepadAim,setGamepadFire,get decalPoolSize(){return decalPool.length;},get decalWrites(){return decalWrites;},get projectilePoolSize(){return 0;},get activeProjectiles(){return 0;},dispose(){stop();window.removeEventListener("arondight:combat-damage",damageListener);window.removeEventListener("arondight:combat-hit-confirm",hitConfirmListener);window.removeEventListener(AUDIO_SETTINGS_EVENT,audioSettingsListener);box3dHitscan.dispose();gamepadCrosshair.remove();damageVignette.remove();for(const mesh of tracerPool)mesh.parent?.remove(mesh);for(const mesh of decalPool)mesh.parent?.remove(mesh);tracerGeometry.dispose();tracerMaterial.dispose();decalGeometry.dispose();decalMaterial.dispose();peerHitProxyMaterial.dispose();for(const el of screenImpacts)el.remove();style.remove();try{audioCtx?.close();}catch{}}};
}
