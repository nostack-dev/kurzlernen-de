...
export function installFlightFireFx({viewport,scene,camera,worldBridge=null,isEnabled=()=>document.body.classList.contains("solo-flight"),onImpact=null}={}){
  ...
  return{stop,get decalPoolSize(){return decalPool.length;},get decalWrites(){return decalWrites;},get impactCount(){return impactSerial;},dispose(){stop();for(const mesh of decalPool){mesh.parent?.remove(mesh);mesh.visible=false;}decalGeometry.dispose();objectDecalMaterial.dispose();worldDecalMaterial.dispose();for(const el of screenImpacts)el.remove();style.remove();try{noiseSource?.stop();}catch{}try{audioCtx?.close();}catch{}}};
}