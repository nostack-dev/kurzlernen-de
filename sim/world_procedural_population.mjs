...
const POPULATION_FOCUS_REFRESH_MS=850;
...
let hitBridge=null,lastFrame=performance.now(),forceImageryOff=false,lastFocusRefresh=-Infinity,focusSignature="";
...
function configureWorld(){
  ...
  if(key===worldKey)return true;anchorX=east;anchorY=north;populationAnchors=[{id:"world-origin",x:east,y:north}];focusSignature="";
  ...
  positionDecor();if(v){v.dataset.worldProceduralSeed=worldSeed.toString(36);v.dataset.worldProceduralBucket=key;v.dataset.worldPopulationMode=key==="training"?"training-physical":"real-road-physical";v.dataset.worldPopulationArchitecture="player-interest-cells+route+box3d-v4";v.dataset.worldLifeArchitecture="player-interest-cells+route+box3d-v4";v.dataset.worldTrafficContinuity="owned-force-driven-contact-resolved-v3";v.dataset.worldCars=String(CAR_COUNT);v.dataset.worldPeople=String(PERSON_COUNT);v.dataset.worldLifeExtraCars=String(CAR_COUNT);v.dataset.worldLifeExtraPeople=String(PERSON_COUNT);v.dataset.worldLifeBuses=String(BUS_COUNT);v.dataset.worldLifeBirds=String(BIRD_COUNT);v.dataset.worldLifeShootable="1";v.dataset.worldTrafficRoutes="1";v.dataset.worldLifeRoutes=String(routes.length);v.dataset.worldVehiclePhysics="box3d-owned-force-controller-v2";v.dataset.worldVehicleGrounding="box3d-static-ground-contact-v1";v.dataset.worldTrainingTrafficRadiusM="52";v.dataset.worldVehicleDamagePresentation="critical-smoke+delayed-explosion-v1";}return true;
}
function selfId(){try{return String(bridge()?.vsSession?.getSelfId?.()||bridge()?.vsSession?.active?.getSelfId?.()||"local");}catch{return"local";}}
function activeFocusPoints(){
  const out=[],seen=new Set(),b=bridge(),sid=selfId(),add=(id,x,y)=>{id=String(id||"");x=Number(x);y=Number(y);if(!id||seen.has(id)||!Number.isFinite(x)||!Number.isFinite(y))return;seen.add(id);out.push({id,x,y});};
  const d=drive(),w=walk();if(d?.active&&d.cameraAnchor)add(sid,d.cameraAnchor.x,d.cameraAnchor.y);else if(w?.mode==="foot"&&w.position)add(sid,w.position.x,w.position.y);else{const air=b?.airframeFor?.(b.threeScene)||b?.airframe;if(air?.position)add(sid,air.position.x,air.position.y);}
  const scene=b?.threeScene;if(scene)scene.traverse?.(node=>{if(node?.visible===false)return;const data=node?.userData||{},id=String(data.vsPlayerId||"");if(!id||id===sid||seen.has(id))return;if(!(data.vsHumanAvatar||data.vsMultiplayerPeer||data.vsLegacyPrimary||data.vsPeer))return;node.getWorldPosition?.(tmp);add(id,tmp.x,tmp.y);});
  return out.sort((a,c)=>a.id.localeCompare(c.id)).slice(0,MAX_POPULATION_FOCI);
}
function refreshPopulationFocus(now=performance.now()){
  if(now-lastFocusRefresh<POPULATION_FOCUS_REFRESH_MS)return false;lastFocusRefresh=now;const points=activeFocusPoints();if(!points.length)return false;const next=points.map(point=>({id:point.id,x:Math.round(point.x/POPULATION_CELL_M)*POPULATION_CELL_M,y:Math.round(point.y/POPULATION_CELL_M)*POPULATION_CELL_M})),signature=next.map(point=>`${point.id}:${point.x}:${point.y}`).join("|");if(signature===focusSignature)return false;focusSignature=signature;populationAnchors=next;anchorX=next.reduce((sum,p)=>sum+p.x,0)/next.length;anchorY=next.reduce((sum,p)=>sum+p.y,0)/next.length;positionDecor();lastRouteRefresh=-Infinity;
  for(const record of records)if((record.kind==="car"||record.kind==="bus")&&record.physicsRegistered&&!record.group.userData.playerDriven&&!record.group.userData.remotePlayerDriven){rigidBodies()?.removeBody?.(record.id);record.physicsRegistered=false;record.physicsPose=null;}
  const v=viewport();if(v){v.dataset.worldPopulationFocus="active-player-cells-v1";v.dataset.worldPopulationFocusCount=String(next.length);v.dataset.worldPopulationFocusCellM=String(POPULATION_CELL_M);v.dataset.worldPopulationFocusSignature=signature;}return true;
}
...
function recordAnchor(record){if(!populationAnchors.length)return{x:anchorX,y:anchorY};return populationAnchors[hashText(record?.id||`${record?.kind}:${record?.index}`)%populationAnchors.length];}
...
function updatePhysicalVehicle(record,epoch,now){
  ...
  let pose=physics?.pose?.(record.id)||record.physicsPose;if(pose?.position?.[2]<-2){physics?.removeBody?.(record.id);record.physicsRegistered=false;pose=null;}const externallyDriven=Boolean(record.group.userData.playerDriven||record.group.userData.remotePlayerDriven);if(!externallyDriven){const current=pose?.position||[initial.x,initial.y,shape.half[2]];let targetPoint;if(route){const nearest=nearestRouteDistance(route,current[0],current[1]);if(nearest.distance>route.length-2.5)record.routeDirection=-1;else if(nearest.distance<2.5)record.routeDirection=1;const lookahead=Math.max(5.5,record.speed*1.15),targetDistance=clamp(nearest.distance+record.routeDirection*lookahead,0,route.length),offset=laneWidth(route,record)*record.routeDirection;targetPoint=sampleRoadRoute(route,targetDistance,offset,record.routeDirection);}else targetPoint=fallbackVehicleSample(record,epoch+1.1);physics?.setTarget?.(record.id,{position:[targetPoint.x,targetPoint.y,shape.half[2]],yaw:targetPoint.yaw,speedMps:record.speed,response:record.kind==="bus"?2.1:2.8,maxAccelerationMps2:record.kind==="bus"?3.4:5.2,owner:"world-ai",priority:10});pose=physics?.pose?.(record.id)||pose;}record.physicsPose=pose;
