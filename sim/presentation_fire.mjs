const DEFAULT_BLOCKED="#soloTopbar,#soloRaceHud,#soloLeft,#soloRight,#soloHeight,#soloActions,#cameraModes,.phone-settings-dialog,.flight-logbook-dialog,[data-world-look-hud],button,input,select,dialog,a,label";
...
export function installPresentationFire({viewport,blockedSelector=DEFAULT_BLOCKED,onShot=()=>{}}={}){
  if(!viewport)throw Error("presentation fire viewport required");installStyle();
  const stick=document.createElement("div");stick.className="presentation-fire-stick";viewport.appendChild(stick);
  const state={pointerId:null,startX:0,startY:0,x:0,y:0,lastShot:-Infinity,timer:0,ctx:null};
  ...
  const shoot=()=>{if(state.pointerId==null)return;const now=performance.now();if(now-state.lastShot<SHOT_INTERVAL_MS-2)return;state.lastShot=now;const p=local(state.x,state.y);flash(p.x,p.y);audioShot(state);try{onShot({clientX:state.x,clientY:state.y,x:p.x,y:p.y});}catch{};};
  const loop=()=>{state.timer=0;if(state.pointerId==null)return;shoot();state.timer=requestAnimationFrame(loop);};
  const begin=event=>{if(state.pointerId!=null||event.button>0||blocked(event.target))return;state.pointerId=event.pointerId;state.startX=state.x=event.clientX;state.startY=state.y=event.clientY;const p=local(event.clientX,event.clientY);stick.style.left=`${p.x}px`;stick.style.top=`${p.y}px`;stick.style.setProperty("--fire-x","0px");stick.style.setProperty("--fire-y","0px");stick.style.display="block";viewport.setPointerCapture?.(event.pointerId);event.preventDefault();shoot();state.timer=requestAnimationFrame(loop);};
  ...
  return{destroy(){if(state.timer)cancelAnimationFrame(state.timer);stick.remove();},get active(){return state.pointerId!=null;}};
}
