...
const localBarrelForward=new THREE.Vector3(0,0,-1),currentBarrelForward=new THREE.Vector3(),shotOrigin=new THREE.Vector3(),shotDirection=new THREE.Vector3(0,1,0),fullAdjust=new THREE.Quaternion();
const worldGunQuat=new THREE.Quaternion(),desiredWorldQuat=new THREE.Quaternion(),parentWorldQuat=new THREE.Quaternion(),inverseParentQuat=new THREE.Quaternion();
const baseGunPosition=new THREE.Vector3(),baseMuzzlePosition=new THREE.Vector3(),rotatedMuzzlePosition=new THREE.Vector3(),desiredMuzzlePosition=new THREE.Vector3(),rootWorldPosition=new THREE.Vector3(),desiredRootWorldPosition=new THREE.Vector3(),verifyMuzzlePosition=new THREE.Vector3(),rayOffset=new THREE.Vector3(),crossError=new THREE.Vector3();
...
const POST_FIRE_ALIGN_MS=190;
const TRANSFORM_EPSILON_M=1e-6;
const QUAT_DOT_EPSILON=1e-7;
let installed=false,lastScreenAimAt=-Infinity,lastScreenFireAt=-Infinity,lastSwitchAt=-Infinity,lastMode="",lastGun=null,screenAimActive=false,lastAimSource="center",hasPresentationSnapshot=false;
...
function restoreOrCaptureBase(gun){
  if(hasPresentationSnapshot&&gun===lastGun&&gun.position.distanceToSquared(lastOutputPosition)<=TRANSFORM_EPSILON_M*TRANSFORM_EPSILON_M&&quaternionMatches(gun.quaternion,lastOutputQuaternion)){gun.position.copy(lastBasePosition);gun.quaternion.copy(lastBaseQuaternion);}else{lastBasePosition.copy(gun.position);lastBaseQuaternion.copy(gun.quaternion);}
  baseGunPosition.copy(gun.position);hasPresentationSnapshot=true;
}
function muzzleNode(gun,mode){if(mode==="smg")return gun.getObjectByName?.("FINAL_MUZZLE_FLASH")||gun.getObjectByName?.("WALK_SMG_MUZZLE_NODE")||gun.getObjectByName?.("WALK_MUZZLE_FLASH");return gun.getObjectByName?.("FINAL_MUZZLE_FLASH")||gun.getObjectByName?.("WALK_MUZZLE_FLASH")||gun.getObjectByName?.("WALK_VM_MUZZLE");}
function setWorldQuaternion(gun,quat){const parent=gun.parent;if(!parent){gun.quaternion.copy(quat);return;}parent.getWorldQuaternion(parentWorldQuat);inverseParentQuat.copy(parentWorldQuat).invert();gun.quaternion.copy(inverseParentQuat.multiply(quat)).normalize();}
function setWorldPosition(gun,position){const parent=gun.parent;if(!parent){gun.position.copy(position);return;}desiredRootWorldPosition.copy(position);parent.worldToLocal(desiredRootWorldPosition);gun.position.copy(desiredRootWorldPosition);}
function verifyCollinearity(gun,muzzle){
  gun.updateMatrixWorld(true);muzzle.getWorldPosition(verifyMuzzlePosition);gun.getWorldQuaternion(worldGunQuat);currentBarrelForward.copy(localBarrelForward).applyQuaternion(worldGunQuat).normalize();rayOffset.copy(verifyMuzzlePosition).sub(shotOrigin);crossError.crossVectors(rayOffset,shotDirection);const lineError=crossError.length(),angleError=Math.acos(clamp(currentBarrelForward.dot(shotDirection),-1,1))*180/Math.PI;return{lineError,angleError};
}
function applyAimTransform(gun,now=performance.now()){
  restoreOrCaptureBase(gun);const fireAge=now-lastScreenFireAt,shouldAlign=screenAimActive||(fireAge>=0&&fireAge<=POST_FIRE_ALIGN_MS),mode=String(footWeapons()?.mode||"pistol");if(!isFoot()||gun.visible===false||!shouldAlign){lastOutputPosition.copy(gun.position);lastOutputQuaternion.copy(gun.quaternion);return false;}
  const muzzle=muzzleNode(gun,mode);if(!muzzle||shotDirection.lengthSq()<.5){lastOutputPosition.copy(gun.position);lastOutputQuaternion.copy(gun.quaternion);return false;}
  gun.updateMatrixWorld(true);muzzle.getWorldPosition(baseMuzzlePosition);gun.getWorldQuaternion(worldGunQuat);currentBarrelForward.copy(localBarrelForward).applyQuaternion(worldGunQuat).normalize();if(currentBarrelForward.lengthSq()<.5){lastOutputPosition.copy(gun.position);lastOutputQuaternion.copy(gun.quaternion);return false;}
  fullAdjust.setFromUnitVectors(currentBarrelForward,shotDirection);desiredWorldQuat.copy(fullAdjust).multiply(worldGunQuat).normalize();setWorldQuaternion(gun,desiredWorldQuat);gun.updateMatrixWorld(true);muzzle.getWorldPosition(rotatedMuzzlePosition);
  const rayDepth=Math.max(.12,baseMuzzlePosition.clone().sub(shotOrigin).dot(shotDirection));desiredMuzzlePosition.copy(shotOrigin).addScaledVector(shotDirection,rayDepth);const dx=desiredMuzzlePosition.x-rotatedMuzzlePosition.x,dy=desiredMuzzlePosition.y-rotatedMuzzlePosition.y,dz=desiredMuzzlePosition.z-rotatedMuzzlePosition.z;gun.getWorldPosition(rootWorldPosition);desiredRootWorldPosition.set(rootWorldPosition.x+dx,rootWorldPosition.y+dy,rootWorldPosition.z+dz);setWorldPosition(gun,desiredRootWorldPosition);gun.updateMatrixWorld(true);
  const verified=verifyCollinearity(gun,muzzle);lastOutputPosition.copy(gun.position);lastOutputQuaternion.copy(gun.quaternion);const view=viewport();if(view){view.dataset.walkWeaponVectorAlignment="muzzle-on-shot-ray+barrel-collinear+hands-rigid-v6";view.dataset.walkWeaponAimVector=`${shotDirection.x.toFixed(4)},${shotDirection.y.toFixed(4)},${shotDirection.z.toFixed(4)}`;view.dataset.walkWeaponAimOrigin=`${shotOrigin.x.toFixed(3)},${shotOrigin.y.toFixed(3)},${shotOrigin.z.toFixed(3)}`;view.dataset.walkWeaponAimActive=screenAimActive?"1":"0";view.dataset.walkWeaponAimSource=lastAimSource;view.dataset.walkWeaponMuzzleRayErrorM=verified.lineError.toFixed(6);view.dataset.walkWeaponBarrelAngleErrorDeg=verified.angleError.toFixed(4);view.dataset.walkWeaponPresentationOwner="scene-before-render-single-transform-v1";}return true;
}
function ensureRenderHook(scene){
  if(!scene)return false;if(hookedScene===scene&&scene.onBeforeRender===renderHook)return true;
  if(hookedScene&&renderHook&&hookedScene.onBeforeRender===renderHook)hookedScene.onBeforeRender=previousSceneOnBeforeRender||(()=>{});
  hookedScene=scene;previousSceneOnBeforeRender=typeof scene.onBeforeRender==="function"?scene.onBeforeRender:null;renderHook=function(...args){previousSceneOnBeforeRender?.apply(this,args);const gun=scene.getObjectByName?.("WALK_PISTOL_3D");if(!gun)return;const mode=String(footWeapons()?.mode||"pistol");setWeaponModeVisual(gun,mode);applyAimTransform(gun,performance.now());};scene.onBeforeRender=renderHook;return true;
}
function updateScreenVector(detail,source,active=true){
  if(!isFoot())return false;const ray=shotRay(Number(detail?.clientX),Number(detail?.clientY));if(!ray)return false;shotOrigin.copy(ray.origin);shotDirection.copy(ray.direction).normalize();lastScreenAimAt=performance.now();screenAimActive=Boolean(active);lastAimSource=String(source||detail?.source||"screen");const view=viewport();if(view){view.dataset.walkWeaponAimVector=`${shotDirection.x.toFixed(4)},${shotDirection.y.toFixed(4)},${shotDirection.z.toFixed(4)}`;view.dataset.walkWeaponAimOrigin=`${shotOrigin.x.toFixed(3)},${shotOrigin.y.toFixed(3)},${shotOrigin.z.toFixed(3)}`;view.dataset.walkWeaponAimActive=screenAimActive?"1":"0";view.dataset.walkWeaponAimSource=lastAimSource;}return true;
}
...
function onScreenFire(event){if(!isFoot())return;const d=event?.detail||{};if(!updateScreenVector(d,d.source||"screen-fire",screenAimActive))return;lastScreenFireAt=performance.now();const view=viewport();if(view){view.dataset.walkWeaponFireVector="shared-touch-screen-ray-collinear-v6";view.dataset.walkWeaponAimSource=String(d.source||"screen");}}
...
function frame(now=performance.now()){
  const scene=bridge()?.threeScene,gun=scene?.getObjectByName?.("WALK_PISTOL_3D"),mode=String(footWeapons()?.mode||"pistol");ensureRenderHook(scene);if(gun){if(gun!==lastGun){hasPresentationSnapshot=false;lastGun=gun;lastMode="";}if(mode!==lastMode)lastMode=mode;setWeaponModeVisual(gun,mode);if(!isFoot())screenAimActive=false;}
  ...
  const view=viewport();if(view){view.dataset.walkWeaponRuntime="dedicated-pistol+mp+collinear-screen-ray-v6";view.dataset.walkTouchFire="screen-point-raycast-v4";view.dataset.walkWeaponAimFreshMs=Number.isFinite(lastScreenAimAt)?String(Math.max(0,Math.round(now-lastScreenAimAt))):"";}requestAnimationFrame(frame);
}
export function installFirstPersonWeaponRuntimeV3(){if(installed)return;installed=true;addEventListener("arondight:foot-screen-aim",onScreenAim);addEventListener("arondight:foot-screen-fire",onScreenFire);addEventListener("arondight:player-mode",()=>{if(!isFoot())screenAimActive=false;hasPresentationSnapshot=false;});window.addEventListener("pointerdown",reliableTouchSwitch,{capture:true,passive:false});window.addEventListener("click",suppressSyntheticClick,{capture:true,passive:false});requestAnimationFrame(frame);}
