...
let installed=false,lastScreenAimAt=-Infinity,lastScreenFireAt=-Infinity,lastSwitchAt=-Infinity,lastMode="",lastGun=null,hasAppliedAdjust=false,screenAimActive=false,lastAimSource="center";
...
function alignGunToShot(gun,now){
  clearPreviousVectorAdjustment(gun);const fireAge=now-lastScreenFireAt;if(!screenAimActive&&(fireAge<0||fireAge>260))return;
  currentBarrelForward.copy(localBarrelForward).applyQuaternion(gun.quaternion).normalize();if(currentBarrelForward.lengthSq()<.5||shotDirection.lengthSq()<.5)return;fullAdjust.setFromUnitVectors(currentBarrelForward,shotDirection);const weight=screenAimActive?1:clamp(1-fireAge/260,0,1);if(weight<=0)return;weightedAdjust.copy(identityQuat).slerp(fullAdjust,weight);lastAppliedAdjust.copy(weightedAdjust);gun.quaternion.premultiply(weightedAdjust);hasAppliedAdjust=true;
  const view=viewport();if(view){view.dataset.walkWeaponVectorAlignment="screen-ray-barrel+hands-live-v5";view.dataset.walkWeaponAimVector=`${shotDirection.x.toFixed(4)},${shotDirection.y.toFixed(4)},${shotDirection.z.toFixed(4)}`;view.dataset.walkWeaponAimWeight=weight.toFixed(3);view.dataset.walkWeaponAimActive=screenAimActive?"1":"0";view.dataset.walkWeaponAimSource=lastAimSource;}
}
function updateScreenVector(detail,source,active=true){
  if(!isFoot())return false;const dir=shotRayDirection(Number(detail?.clientX),Number(detail?.clientY));if(!dir)return false;shotDirection.copy(dir).normalize();lastScreenAimAt=performance.now();screenAimActive=Boolean(active);lastAimSource=String(source||detail?.source||"screen");const view=viewport();if(view){view.dataset.walkWeaponAimVector=`${shotDirection.x.toFixed(4)},${shotDirection.y.toFixed(4)},${shotDirection.z.toFixed(4)}`;view.dataset.walkWeaponAimActive=screenAimActive?"1":"0";view.dataset.walkWeaponAimSource=lastAimSource;}return true;
}
...
function onScreenFire(event){if(!isFoot())return;const d=event?.detail||{};if(!updateScreenVector(d,d.source||"screen-fire",screenAimActive))return;lastScreenFireAt=performance.now();const view=viewport();if(view){view.dataset.walkWeaponFireVector="shared-touch-screen-ray-v5";view.dataset.walkWeaponAimSource=String(d.source||"screen");}}
...
function frame(now=performance.now()){
  ...
  const view=viewport();if(view){view.dataset.walkWeaponRuntime="dedicated-pistol+mp+live-screen-vector-v5";view.dataset.walkTouchFire="screen-point-raycast-v4";view.dataset.walkWeaponAimFreshMs=Number.isFinite(lastScreenAimAt)?String(Math.max(0,Math.round(now-lastScreenAimAt))):"";}requestAnimationFrame(frame);
}
export function installFirstPersonWeaponRuntimeV3(){if(installed)return;installed=true;addEventListener("arondight:foot-screen-aim",onScreenAim);addEventListener("arondight:foot-screen-fire",onScreenFire);addEventListener("arondight:player-mode",()=>{if(!isFoot())screenAimActive=false;});window.addEventListener("pointerdown",reliableTouchSwitch,{capture:true,passive:false});window.addEventListener("click",suppressSyntheticClick,{capture:true,passive:false});requestAnimationFrame(frame);}
