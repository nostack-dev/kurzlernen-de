...
class DirectUdpRoom{
  constructor(config,roomId,callbacks={}){
    this.config=config||{};this.appId=clean(config?.appId||"app");this.roomId=clean(roomId);this.tag=`a45-vs-p2p4-${this.appId}-${this.roomId}`;this.id=randomId();this.peerId="";this.peerPubkey="";this.peer=null;this.actions=new Map();this.pendingProbes=new Map();this.seen=new Set();this.closed=false;this.ready=false;this._onPeerJoin=null;this._onPeerLeave=null;this.onJoinError=callbacks?.onJoinError;this.helloTimer=0;this.heartbeatTimer=0;this.errorTimer=0;
  ...
  _armDiscovery(){if(this.closed)return;clearInterval(this.helloTimer);clearInterval(this.heartbeatTimer);clearTimeout(this.errorTimer);this.helloTimer=setInterval(()=>this._announce(),HELLO_MS);this.heartbeatTimer=setInterval(()=>{if(!this.ready)this._announce();},HEARTBEAT_MS);this.errorTimer=setTimeout(()=>{if(this.closed||this.ready)return;const error=Error(this.peerId?"Direct UDP peer connection timed out":"Direct UDP peer not found");emitNetwork("direct-udp-timeout",{roomId:this.roomId,peerId:this.peerId,relays:getRelaySockets()});this.onJoinError?.({peerId:this.peerId,error});},CONNECT_ERROR_MS);queueMicrotask(()=>this._announce());}
  ...
  _probe(peerId,pubkey){if(this.closed||this.peerId||!peerId||peerId===this.id||!pubkey)return;const previous=this.pendingProbes.get(peerId),now=Date.now();if(previous&&previous.pubkey===pubkey&&now-previous.at<750)return;const nonce=packetId();this.pendingProbes.set(peerId,{nonce,pubkey,at:now});for(const [id,probe] of this.pendingProbes)if(now-probe.at>PROBE_TTL_MS)this.pendingProbes.delete(id);this._publishSignal({...this._base("probe",peerId),nonce}).catch(()=>{});emitNetwork("signal-probe",{roomId:this.roomId,peerId});}
  ...
  async _receiveSignalAsync(packet){if(this.closed||packet.id===this.id||!packet._pubkey||!this._remember(packet.msgId))return;if(packet.target&&packet.target!==this.id)return;
    if(packet.kind==="hello"){if(!this.peerId)this._probe(packet.id,packet._pubkey);return;}
    ...
    if(packet.kind==="probe-ack"){const probe=this.pendingProbes.get(packet.id);if(this.peerId||!probe||probe.nonce!==packet.nonce||probe.pubkey!==packet._pubkey||Date.now()-probe.at>PROBE_TTL_MS)return;this.pendingProbes.clear();await this._adoptPeer(packet.id,packet._pubkey);return;}
    if(!this.peerId||packet.id!==this.peerId||packet._pubkey!==this.peerPubkey)return;if(packet.kind==="bye"){this._peerGone("signal-bye");return;}if(["description","candidate","restart-needed"].includes(packet.kind))await this.peer?.handleSignal({kind:packet.kind,...signalPayload(packet)});
  }
  async _adoptPeer(peerId,pubkey){if(this.closed||this.peer||!peerId||!pubkey)return;this.peerId=peerId;this.peerPubkey=pubkey;emitNetwork("authority-elected",{roomId:this.roomId,peerId,role:this.id<peerId?"host":"client"});const peer=new DirectUdpPeer({localId:this.id,peerId,RTCPeerConnectionCtor:this.config?.RTCPeerConnectionCtor,iceServers:this.config?.iceServers,sendSignal:signal=>this._publishSignal({...this._base(signal.kind,peerId),...signalPayload(signal)}),onReady:()=>{if(this.closed||peer!==this.peer)return;this.ready=true;this._stopDiscoveryTimers();emitNetwork("direct-udp-ready",{roomId:this.roomId,peerId,role:peer.isHost?"host":"client"});this._onPeerJoin?.(peerId);},onLeave:(_id,reason)=>{if(peer===this.peer)this._peerGone(reason||"peer-engine-left");},onAction:(action,data)=>{if(this.closed||peer!==this.peer||!this.ready)return;this.actions.get(String(action||""))?.onMessage?.(data,{peerId});},onDiagnostic:detail=>emitNetwork(detail.stage,{roomId:this.roomId,...detail})});this.peer=peer;try{await peer.start();}catch(error){this._peerGone("peer-engine-start-error");throw error;}}
  _peerGone(reason){if(this.closed)return;const peerId=this.peerId,wasReady=this.ready;this.ready=false;this.peerId="";this.peerPubkey="";this.pendingProbes.clear();const peer=this.peer;this.peer=null;try{peer?.close?.(reason,false);}catch{}if(wasReady&&peerId)this._onPeerLeave?.(peerId);emitNetwork("direct-udp-peer-left",{roomId:this.roomId,peerId,reason});this._armDiscovery();}
  makeAction(name){const key=String(name||"");const action={onMessage:null,send:(data,{target}={})=>{if(!this.ready||!this.peer||!this.peerId)return Promise.reject(Error("Direct UDP peer unavailable"));if(target&&String(target)!==this.peerId)return Promise.reject(Error("Direct UDP target peer unavailable"));return this.peer.sendAction(key,data);}};this.actions.set(key,action);return action;}
  getPeers(){return this.ready&&this.peerId&&this.peer?.pc?{[this.peerId]:this.peer.pc}:{};}
  ping(peerId){if(!this.ready||peerId!==this.peerId||!this.peer)return Promise.reject(Error("Direct UDP peer unavailable"));return this.peer.ping();}
  leave(){if(this.closed)return;const goodbye=this.peerId?this._publishSignal(this._base("bye",this.peerId)).catch(()=>{}):Promise.resolve();this.closed=true;this._stopDiscoveryTimers();try{this.subscription?.close?.("room leave");}catch{}try{this.peer?.close?.("room leave",false);}catch{}this.peer=null;this.ready=false;this.peerId="";this.peerPubkey="";this.pendingProbes.clear();this.actions.clear();rooms.delete(this);goodbye.finally(()=>closePoolIfIdle());}
}
