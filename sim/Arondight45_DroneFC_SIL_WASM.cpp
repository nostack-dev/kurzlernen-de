 ...
 *
 * The browser never calls StateRuntime with fabricated state. It emits the same raw
 * hardware frames that production obtains from SPI/UART, and this adapter executes
 * the exact FirmwareRuntime -> StateRuntime -> Runtime stack used by the S31 target.
 */
...
extern "C" {
SIL_EXPORT unsigned char* fc_input_buffer() { return reinterpret_cast<unsigned char*>(&input_packet); }
SIL_EXPORT const unsigned char* fc_output_buffer() { return reinterpret_cast<const unsigned char*>(&output_packet); }
SIL_EXPORT uint32_t fc_input_size() { return static_cast<uint32_t>(sizeof(input_packet)); }
SIL_EXPORT uint32_t fc_output_size() { return static_cast<uint32_t>(sizeof(output_packet)); }
...
SIL_EXPORT uint32_t fc_protocol_version() { return hil::kProtocolVersion; }
}
...
void check(bool condition, const char* what) {
    if (!condition) { std::fprintf(stderr, "SIL FAIL: %s\n", what); std::exit(1); }
}
void encode_sbus(const std::array<uint16_t, 16>& channels, uint8_t* p) {
    std::memset(p, 0, 25); p[0] = 0x0F; p[24] = 0x00;
    for (int channel = 0; channel < 16; ++channel) for (int bit = 0; bit < 11; ++bit) {
...
void finalize(hil::InputPacket& in) {
    in.crc32 = hil::crc32(&in, offsetof(hil::InputPacket, crc32));
}
}

int main() {
    check(fc_input_size() == 80, "input packet size");
    check(fc_output_size() == 32, "output packet size");
    check(fc_protocol_version() == 3, "protocol version");
    ...
    in->dt_us = 1000;
    in->flags = hil::kFlagImuPresent | hil::kFlagSbusPresent | hil::kFlagReset;
    in->imu_registers[6] = 0x08;  // +1 g Z at target ICM scale.

    std::array<uint16_t, 16> channels{};
    channels.fill(992); channels[2] = 172; channels[4] = 172;
    encode_sbus(channels, in->sbus);

    for (uint32_t i = 0; i < fc::kCalibrationSamples + 1; ++i) {
        in->sequence = i;
        in->flags = hil::kFlagImuPresent | hil::kFlagSbusPresent |
                    (i == 0 ? hil::kFlagReset : 0);
        finalize(*in); fc_process();
    ...
    channels[4] = 1811; encode_sbus(channels, in->sbus);
    for (uint32_t i = 0; i < 1002; ++i) {
        ++in->sequence; in->flags = hil::kFlagImuPresent | hil::kFlagSbusPresent;
        finalize(*in); fc_process();
    }
    ...
    channels[2] = 700; encode_sbus(channels, in->sbus); ++in->sequence;
    finalize(*in); fc_process();
    for (uint16_t pulse : out->motor_us) check(pulse > fc::kEscIdleUs, "throttle produces thrust command");

    std::puts("All Arondight45 SIL raw-hardware-runtime tests passed.");
    return 0;
}
#endif
