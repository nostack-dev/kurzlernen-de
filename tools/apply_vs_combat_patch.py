...
def replace1(path, old, new):
    ...
    s = p.read_text()
    n = s.count(old)
    assert n == 1, f"{path}: expected one match, got {n}: {old[:120]!r}"
    p.write_text(s.replace(old, new, 1))


# transport: separate reliable combat action
path = "sim/lan_vs.mjs"
replace1(
    path,
...
replace1(path, 'this.room=null;this.poseAction=null;this.originAction=null;this.timer=0;', 'this.room=null;this.poseAction=null;this.originAction=null;this.combatAction=null;this.timer=0;')
replace1(path, 'this.onPeer=options.onPeer;this.onPose=options.onPose;this.onOrigin=options.onOrigin;this.onLeave=options.onLeave;this.onError=options.onError;this.onTransport=options.onTransport;', 'this.onPeer=options.onPeer;this.onPose=options.onPose;this.onOrigin=options.onOrigin;this.onCombat=options.onCombat;this.onLeave=options.onLeave;this.onError=options.onError;this.onTransport=options.onTransport;')
replace1(path, 'this.room=null;this.poseAction=null;this.originAction=null;this.transportName="";', 'this.room=null;this.poseAction=null;this.originAction=null;this.combatAction=null;this.transportName="";')
replace1(path, '    this.originAction=originAction;\n    const active=()=>generation===this.transportGeneration&&room===this.room;', '    this.originAction=originAction;\n    const combatAction=room.makeAction("combat");\n    if(!combatAction||typeof combatAction.send!=="function")throw Error("VS combat action unavailable");\n    this.combatAction=combatAction;\n    const active=()=>generation===this.transportGeneration&&room===this.room;')
replace1(path, '    originAction.onMessage=(origin,{peerId}={})=>{if(!active()||!peerId||peerId!==this.peerId||!validOrigin(origin))return;this.onOrigin?.({...origin},peerId);};\n    poseAction.onMessage=(pose,{peerId}={})=>{', '    originAction.onMessage=(origin,{peerId}={})=>{if(!active()||!peerId||peerId!==this.peerId||!validOrigin(origin))return;this.onOrigin?.({...origin},peerId);};\n    combatAction.onMessage=(packet,{peerId}={})=>{if(!active()||!peerId||!validCombat(packet))return;if(!this.peerId)adoptPeer(peerId);if(peerId!==this.peerId)return;this.onCombat?.({...packet},peerId);};\n    poseAction.onMessage=(pose,{peerId}={})=>{')
replace1(path, '  stop(){\n    clearInterval(this.timer);', '  sendCombat(packet){\n    if(!validCombat(packet)||!this.peerId||!this.combatAction)return false;\n    Promise.resolve(this.combatAction.send({...packet},{target:this.peerId})).catch(error=>this.onError?.(error));\n    return true;\n  }\n  stop(){\n    clearInterval(this.timer);')
replace1(path, 'this.room=null;this.poseAction=null;this.originAction=null;this.peerId=null;', 'this.room=null;this.poseAction=null;this.originAction=null;this.combatAction=null;this.peerId=null;')

# bridge: HP, mate HP, kill count, explosion, respawn
path = "sim/real_world_bootstrap.mjs"
replace1(path, 'this.vsPeerLastPoseMs=-Infinity;this.vsPeerRenderMs=performance.now();this.installUi();', 'this.vsPeerLastPoseMs=-Infinity;this.vsPeerRenderMs=performance.now();this.vsCombatHud=null;this.vsLocalHealth=100;this.vsPeerHealth=100;this.vsKills=0;this.vsDeaths=0;this.vsCombatSeq=0;this.vsSeenHits=new Set();this.vsPendingHits=new Set();this.vsRespawnTimer=0;this.vsLocalDead=false;this.vsPeerDead=false;this.vsExplosion=null;this.vsExplosionStartedMs=-Infinity;this.installUi();')
replace1(
    path,
    '''  installVsUi(){
...
replace1(path, 'onPeer:()=>{this.vsConnected=true;if(button)button.textContent="MATE ✓";this.ensureVsPeerMesh();},onPose:pose=>this.applyVsPose(pose),onOrigin:origin=>this.acceptVsOrigin(origin,null),onLeave:()=>{', 'onPeer:()=>{this.vsConnected=true;if(button)button.textContent="MATE ✓";this.ensureVsPeerMesh();this.resetVsCombat(true);},onPose:pose=>this.applyVsPose(pose),onOrigin:origin=>this.acceptVsOrigin(origin,null),onCombat:packet=>this.applyVsCombat(packet),onLeave:()=>{')
replace1(path, 'this.clearVsPeerPresentation();if(mateWorld){', 'this.clearVsPeerPresentation();this.resetVsCombat(false);if(mateWorld){')
replace1(path, 'this.vsSession?.stop();this.vsSession=null;if(button)button.textContent="RETRY MATE";', 'this.vsSession?.stop();this.vsSession=null;this.resetVsCombat(false);if(button)button.textContent="RETRY MATE";')
replace1(
    path,
...
replace1(path, '  updateVsPeerRender(){\n    if(!this.vsPeerMesh||!this.vsPeerTargetPosition||!this.vsPeerTargetQuaternion)return;', '  updateVsPeerRender(){\n    this.updateVsExplosion();if(!this.vsPeerMesh||!this.vsPeerTargetPosition||!this.vsPeerTargetQuaternion)return;if(this.vsPeerDead){this.vsPeerMesh.visible=false;return;}')
replace1(path, 'group.visible=false;group.renderOrder=5;this.threeScene.add(group);this.vsPeerMesh=group;', 'group.userData.vsPeer=true;group.visible=false;group.renderOrder=5;this.threeScene.add(group);this.vsPeerMesh=group;')
replace1(path, '  stopVs(){this.vsStarting=false;const mateWorld=this.vsWorldFromMate;this.vsSession?.stop();this.vsSession=null;this.vsConnected=false;this.vsSharedOrigin=null;this.vsSharedWorldAttempted=false;this.vsWorldFromMate=false;this.clearVsPeerPresentation();if(mateWorld){if(this.active)this.deactivate();this.originLon=null;this.originLat=null;}const b=$("lanVsButton");if(b)b.textContent="FIND MATE";}', '  stopVs(){this.vsStarting=false;const mateWorld=this.vsWorldFromMate;this.vsSession?.stop();this.vsSession=null;this.vsConnected=false;this.vsSharedOrigin=null;this.vsSharedWorldAttempted=false;this.vsWorldFromMate=false;this.clearVsPeerPresentation();this.resetVsCombat(false);if(mateWorld){if(this.active)this.deactivate();this.originLon=null;this.originLat=null;}const b=$("lanVsButton");if(b)b.textContent="FIND MATE";}')

# gun raycast: only a visible peer can be hit; no decal frozen in space on moving peer
path = "sim/flight_fire_fx.mjs"
replace1(path, '  function hiddenTrainingObject(object){if(!worldBridge?.active)return false;for(let node=object;node;node=node.parent)if(worldBridge.trainingObjects?.has?.(node))return true;return false;}\n', '  function hiddenTrainingObject(object){if(!worldBridge?.active)return false;for(let node=object;node;node=node.parent)if(worldBridge.trainingObjects?.has?.(node))return true;return false;}\n  function visibleInHierarchy(object){for(let node=object;node;node=node.parent)if(node.visible===false)return false;return true;}\n')
replace1(path, 'scene.traverse(object=>{if(object.isMesh&&object.visible&&!object.userData?.arondightAirframe&&!object.userData?.flightFireDecal&&object.material?.visible!==false&&!hiddenTrainingObject(object))candidates.push(object);});', 'scene.traverse(object=>{if(object.isMesh&&visibleInHierarchy(object)&&!object.userData?.arondightAirframe&&!object.userData?.flightFireDecal&&!object.userData?.flightFireIgnore&&object.material?.visible!==false&&!hiddenTrainingObject(object))candidates.push(object);});')
replace1(path, '    if(hit)addThreeDecal(hit);else{const worldHit=worldBridge?.addVisualShotImpact?.(aim.x,aim.y,aim.rect,raycaster.ray);if(worldHit)addThreeDecal(worldHit);}\n    screenImpact(aim.x,aim.y);shotSound();viewport.dataset.fireShots=', '    const vsHit=Boolean(hit&&worldBridge?.registerVsHit?.(hit));if(hit&&!vsHit)addThreeDecal(hit);else if(!hit){const worldHit=worldBridge?.addVisualShotImpact?.(aim.x,aim.y,aim.rect,raycaster.ray);if(worldHit)addThreeDecal(worldHit);}if(vsHit)viewport.dataset.fireVsHits=String((Number(viewport.dataset.fireVsHits)||0)+1);\n    screenImpact(aim.x,aim.y);shotSound();viewport.dataset.fireShots=')

# compact HUD styling
path = "sim/solo_layout.mjs"
replace1(path, '    body.solo-flight #soloTopbar #soloCamera,body.solo-flight #soloTopbar .phone-settings-button,body.solo-flight #soloTopbar #lanVsButton{display:inline-flex!important;flex:0 0 auto;min-width:58px;min-height:28px;align-items:center;justify-content:center}\n', '    body.solo-flight #soloTopbar #soloCamera,body.solo-flight #soloTopbar .phone-settings-button,body.solo-flight #soloTopbar #lanVsButton{display:inline-flex!important;flex:0 0 auto;min-width:58px;min-height:28px;align-items:center;justify-content:center}\n    body.solo-flight #soloTopbar #vsCombatHud{display:inline-flex;align-items:center;justify-content:center;min-width:112px;font-size:10px;font-weight:900;font-variant-numeric:tabular-nums;white-space:nowrap}\n    body.solo-flight #soloTopbar #vsCombatHud[hidden]{display:none!important}\n')
replace1(path, '      body.solo-flight #soloTopbar #soloCamera,body.solo-flight #soloTopbar .phone-settings-button,body.solo-flight #soloTopbar #lanVsButton{min-width:52px;min-height:24px}\n', '      body.solo-flight #soloTopbar #soloCamera,body.solo-flight #soloTopbar .phone-settings-button,body.solo-flight #soloTopbar #lanVsButton{min-width:52px;min-height:24px}\n      body.solo-flight #soloTopbar #vsCombatHud{min-width:100px;font-size:9px;padding-left:5px;padding-right:5px}\n')

# deterministic transport tests include combat both ways
path = "tests/lan_vs_smoke.mjs"
replace1(path, 'let aPeer=0,bPeer=0,aPose=null,bPose=null,aOrigin=null,bOrigin=null,aLeft=0;\nconst a=new LanVsSession({...harness,onPeer:()=>aPeer++,onPose:p=>aPose=p,onOrigin:o=>aOrigin=o,onLeave:()=>aLeft++});\nconst b=new LanVsSession({...harness,onPeer:()=>bPeer++,onPose:p=>bPose=p,onOrigin:o=>bOrigin=o});', 'let aPeer=0,bPeer=0,aPose=null,bPose=null,aOrigin=null,bOrigin=null,aCombat=null,bCombat=null,aLeft=0;\nconst a=new LanVsSession({...harness,onPeer:()=>aPeer++,onPose:p=>aPose=p,onOrigin:o=>aOrigin=o,onCombat:p=>aCombat=p,onLeave:()=>aLeft++});\nconst b=new LanVsSession({...harness,onPeer:()=>bPeer++,onPose:p=>bPose=p,onOrigin:o=>bOrigin=o,onCombat:p=>bCombat=p});')
replace1(path, 'assert.ok(aPose.seq>oldSeq,"pose sequence did not advance");\nb.stop();', 'assert.ok(aPose.seq>oldSeq,"pose sequence did not advance");\nassert.equal(a.sendCombat({type:"hit",id:"hit-1",damage:25}),true);await new Promise(r=>setTimeout(r,5));assert.deepEqual(bCombat,{type:"hit",id:"hit-1",damage:25});\nassert.equal(b.sendCombat({type:"state",id:"hit-1",hp:75,killed:false}),true);await new Promise(r=>setTimeout(r,5));assert.deepEqual(aCombat,{type:"state",id:"hit-1",hp:75,killed:false});\nassert.equal(a.sendCombat({type:"hit",id:"",damage:25}),false);assert.equal(a.sendCombat({type:"hit",id:"bad",damage:101}),false);\nassert.equal(b.sendCombat({type:"respawn",hp:100}),true);await new Promise(r=>setTimeout(r,5));assert.deepEqual(aCombat,{type:"respawn",hp:100});\nb.stop();')
replace1(path, 'console.log("LAN VS deterministic smoke passed: automatic same-NAT room, no pair code, and Nostr -> MQTT signaling fallback");', 'console.log("LAN VS deterministic smoke passed: automatic same-NAT room, Nostr -> MQTT fallback, pose/origin and reliable combat actions");')

# permanent architecture contract
path = "tests/architecture_invariants.mjs"
replace1(path, 'requireText("sim/lan_vs.mjs","room.makeAction(\\\"origin\\\")","VS must exchange shared origin independently from pose");', 'requireText("sim/lan_vs.mjs","room.makeAction(\\\"origin\\\")","VS must exchange shared origin independently from pose");\nrequireText("sim/lan_vs.mjs","room.makeAction(\\\"combat\\\")","VS combat must use a separate P2P action from pose/origin");\nrequireText("sim/real_world_bootstrap.mjs","registerVsHit(hit)","peer hit detection must stay in the VS presentation/game layer");\nrequireText("sim/real_world_bootstrap.mjs","this.vsLocalHealth=100","VS must track explicit player health");\nrequireText("sim/real_world_bootstrap.mjs","this.vsKills++","VS must count victim-authoritative kills");\nrequireText("sim/real_world_bootstrap.mjs","explodeVsPeer()","VS kill must trigger peer explosion presentation");\nrequireText("tests/vs_combat_browser_smoke.mjs","duplicate hit changed health twice","browser gate must cover combat dedupe and kill presentation");\nfor(const fcPath of ["esp32/Arondight45_DroneFC_Core.hpp","esp32/Arondight45_StateControl.hpp","esp32/Arondight45_FirmwareRuntime.hpp","sim/Arondight45_DroneFC_SIL_WASM.cpp"])forbidText(fcPath,"vsCombat","VS combat must never enter shared FC/physics authority");')

# layout gate: combat HUD belongs to topbar and starts hidden
path = "tests/solo_layout_smoke.mjs"
replace1(path, 'topbar:rect("#soloTopbar"),vs:rect("#lanVsButton"),vsParent:document.querySelector("#lanVsButton")?.parentElement?.id||"",left:rect("#soloLeft")', 'topbar:rect("#soloTopbar"),vs:rect("#lanVsButton"),vsParent:document.querySelector("#lanVsButton")?.parentElement?.id||"",vsCombatParent:document.querySelector("#vsCombatHud")?.parentElement?.id||"",vsCombatHidden:Boolean(document.querySelector("#vsCombatHud")?.hidden),left:rect("#soloLeft")')
replace1(path, 'if(g.vsParent!=="soloTopbar")throw new Error(`${viewport.name}: FIND MATE is not in the topbar: ${JSON.stringify({parent:g.vsParent,vs:g.vs,topbar:g.topbar})}`);', 'if(g.vsParent!=="soloTopbar")throw new Error(`${viewport.name}: FIND MATE is not in the topbar: ${JSON.stringify({parent:g.vsParent,vs:g.vs,topbar:g.topbar})}`);\n    if(g.vsCombatParent!=="soloTopbar"||!g.vsCombatHidden)throw new Error(`${viewport.name}: VS combat HUD lifecycle/layout invalid: ${JSON.stringify({parent:g.vsCombatParent,hidden:g.vsCombatHidden})}`);')

Path("tests/vs_combat_browser_smoke.mjs").write_text(r'''import puppeteer from "puppeteer-core";
...
# production deploy permanently runs combat browser gate
path = ".github/workflows/deploy.yml"
replace1(path, '          CHROME_BIN="$CHROME_BIN" node tests/solo_layout_smoke.mjs http://127.0.0.1:4174\n          CHROME_BIN="$CHROME_BIN" node tests/real_world_ui_smoke.mjs', '          CHROME_BIN="$CHROME_BIN" node tests/solo_layout_smoke.mjs http://127.0.0.1:4174\n          CHROME_BIN="$CHROME_BIN" node tests/vs_combat_browser_smoke.mjs http://127.0.0.1:4174\n          CHROME_BIN="$CHROME_BIN" node tests/real_world_ui_smoke.mjs')
replace1(path, '          node --check tests/solo_layout_smoke.mjs\n          node --check tests/real_world_ui_smoke.mjs', '          node --check tests/solo_layout_smoke.mjs\n          node --check tests/vs_combat_browser_smoke.mjs\n          node --check tests/real_world_ui_smoke.mjs')
