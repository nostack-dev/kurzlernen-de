...
def replace_once(path, old, new):
    ...
    if count != 1:
        raise RuntimeError(f"{path}: expected one exact pattern, found {count}")
...
def regex_once(path, pattern, replacement):
    ...
    if count != 1:
        raise RuntimeError(f"{path}: regex matched {count}")
    p.write_text(updated)


# ======================== production ESP32-S31 ==============================
...
# Production SPI returns the literal target register block. Scaling/orientation is
# owned by Arondight45_HardwareSensors.hpp and therefore cannot diverge from SIL/HIL.
regex_once(p,
    r'int16_t read_be_i16\(const uint8_t\* p\) \{.*?esp_err_t sample_imu\(fc::Imu& sample\) \{.*?\n\}',
'''esp_err_t sample_imu_registers(std::array<uint8_t, 14>& sample) {
    return imu_read(reg::kTempData1, sample.data(), sample.size());
}''')

# Receiver and navigation inputs remain raw UART frames until FirmwareRuntime.
regex_once(p,
    r'esp_err_t sbus_init\(\) \{.*?fc::RC get_rc_snapshot\(\) \{.*?\n\}',
'''esp_err_t sbus_init() {
...
replace_once(p, 'function parseOutput(bytes) {', r'''function crc16Ccitt(bytes,length=bytes.byteLength){
  let crc=0xffff;
...
regex_once(p,
    r'function makeInput\(sequence, imu, sbus, flags, dtUs=1000, navigation=null\) \{.*?\n\}\n\nclass WasmBackend',
...
regex_once(p,
    r'// Navigation truth is never fed directly to control\..*?\nclass SimNavigationSensors \{.*?\n\}\n\nconst b3 = await Box3DFactory\(\);',
...
let latestNavigation=''')
replace_once(p,
'''  physics.reset(defaultParams(),initial);navigationSensors.reset();latestNavigation=''',
...
replace_once(p, needle, needle + '''requireText("esp32/Arondight45_DroneFC_S31.cpp","navigation_init");
...
print("Applied no-cheat raw-hardware digital-twin migration v2")
